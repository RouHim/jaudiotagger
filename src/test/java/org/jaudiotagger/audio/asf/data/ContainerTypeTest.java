...
public class ContainerTypeTest {
    ...
    public void testAreInCorrectOrder() {
        for (int i = 0; i < ContainerType.values().length; i++) {
            for (int j = i + 1; j < ContainerType.values().length; j++) {
                assertTrue(
                        ContainerType.areInCorrectOrder(
                                ContainerType.getOrdered()[i],
                                ContainerType.getOrdered()[j]
                        )
                );
            }
            for (int j = 0; j < i; j++) {
    ...
    public void testCheckCtonstraints() {
        assertNotNull(
                ContainerType.CONTENT_DESCRIPTION.checkConstraints(null, null, 0, 0, 0)
        );
        assertNotNull(
                ContainerType.CONTENT_DESCRIPTION.checkConstraints("name", null, 0, 0, 0)
        );
        assertNotNull(
                ContainerType.CONTENT_DESCRIPTION.checkConstraints(
                        createAString(65536),
                        new byte[0],
                        0,
                        0,
        ...
        assertNull(
                ContainerType.METADATA_LIBRARY_OBJECT.checkConstraints(
                        "name",
                        createAString(30).getBytes(),
                        5,
                        0,
        ...
        assertNotNull(
                ContainerType.METADATA_OBJECT.checkConstraints(
                        "name",
                        createAString(30).getBytes(),
                        MetadataDescriptor.TYPE_BOOLEAN,
                        0,
    ...
    public void testGetMaximumDataLength() {
        ...
        assertEquals(
                wordSize,
                ContainerType.CONTENT_DESCRIPTION.getMaximumDataLength()
        );
        assertEquals(
                dwordSize,
                ContainerType.CONTENT_BRANDING.getMaximumDataLength()
        );
        assertEquals(
                wordSize,
                ContainerType.EXTENDED_CONTENT.getMaximumDataLength()
        );
        assertEquals(
                wordSize,
                ContainerType.METADATA_OBJECT.getMaximumDataLength()
        );
        assertEquals(
                dwordSize,
                ContainerType.METADATA_LIBRARY_OBJECT.getMaximumDataLength()
        );
    }
    ...
    public void testIsWithinThanValueRange() {
        ...
        assertTrue(
                ContainerType.CONTENT_DESCRIPTION.isWithinValueRange(wordSize.longValue())
        );
        assertFalse(ContainerType.CONTENT_DESCRIPTION.isWithinValueRange(-1));
        assertFalse(
                ContainerType.CONTENT_DESCRIPTION.isWithinValueRange(
                        (wordSize.longValue() + 1L)
                )
        );
        // Content Branding
        ...
        assertFalse(
                ContainerType.CONTENT_BRANDING.isWithinValueRange(
                        (dwordSize.longValue() + 1L)
                )
        );
        // Extended Content Description
        assertTrue(ContainerType.EXTENDED_CONTENT.isWithinValueRange(0));
        assertTrue(
                ContainerType.EXTENDED_CONTENT.isWithinValueRange(wordSize.longValue())
        );
        assertFalse(ContainerType.EXTENDED_CONTENT.isWithinValueRange(-1));
        assertFalse(
                ContainerType.EXTENDED_CONTENT.isWithinValueRange(
                        (wordSize.longValue() + 1L)
                )
        );
        // Metadata Object
        ...
        assertTrue(
                ContainerType.METADATA_OBJECT.isWithinValueRange(wordSize.longValue())
        );
        assertFalse(ContainerType.METADATA_OBJECT.isWithinValueRange(-1));
        assertFalse(
                ContainerType.METADATA_OBJECT.isWithinValueRange(
                        (wordSize.longValue() + 1L)
                )
        );
        ...
        assertTrue(
                ContainerType.METADATA_LIBRARY_OBJECT.isWithinValueRange(
                        dwordSize.longValue()
                )
        );
