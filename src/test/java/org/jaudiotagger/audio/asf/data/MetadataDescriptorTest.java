...
public class MetadataDescriptorTest {
   ...
   */
  protected void setStringFail(
    final MetadataDescriptor desc,
    final String value
  ) {
    try {
  ...
  public void testCreateCopy() {
    ...
    for (ContainerType curr : ContainerType.getOrdered()) {
      ...
      int lang = ContainerType.areInCorrectOrder(
        ...
        ? rand.nextInt(5) + 1
        : 0;
      final MetadataDescriptor orig = new MetadataDescriptor(
        curr,
        "name",
  ...
  public void testEqualsObject() {
    ...
    for (int i = 0; i < types.length; i++) {
      MetadataDescriptor one = new MetadataDescriptor(
        types[i],
        types[i].name(),
        MetadataDescriptor.TYPE_STRING,
        types[i].isStreamNumberEnabled() ? 3 : 0,
        types[i].isLanguageEnabled() ? 3 : 0
      );
      one.setStringValue(createAString(i));
      assertNotEquals(null, one);
      assertNotEquals(one, new Object());
      for (int j = 0; j < types.length; j++) {
        MetadataDescriptor two = new MetadataDescriptor(
          types[j],
          types[j].name(),
          MetadataDescriptor.TYPE_STRING,
          types[j].isStreamNumberEnabled() ? 3 : 0,
          types[j].isLanguageEnabled() ? 3 : 0
        );
  ...
  public void testGetRawDataSize() {
    ...
    assertEquals(
      this.descriptor1.getRawData().length,
      this.descriptor1.getRawDataSize()
    );
  }
  ...
  public void testSetBinaryValue() {
    ...
    assertArrayEquals(test, desc.getRawData());
    // Test size violations.
    desc = new MetadataDescriptor(
  ...
  public void testSetString() {
    ...
    setStringFail(
      desc,
      MetadataDescriptor.QWORD_MAXVALUE.add(BigInteger.ONE).toString(10)
    ); // too big.
    setStringFail(desc, String.valueOf(-1)); // too small
    ...
    setStringFail(desc, "");
    setStringFail(desc, GUID.GUID_AUDIOSTREAM + "a");
  }
