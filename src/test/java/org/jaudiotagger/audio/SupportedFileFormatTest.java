...
public class SupportedFileFormatTest {
  ...
  public void testFromExtension() {
    ...
    for (String extension : expectedMap.keySet()) {
      assertSame(
        expectedMap.get(extension),
        SupportedFileFormat.fromExtension(extension)
      );
    }
  }
  ...
  public void testCreateDefaultTag() {
    EnumSet<SupportedFileFormat> formatSet = EnumSet.allOf(
      SupportedFileFormat.class
    ); // ensure we test all
    defaultTagIsInstanceOf(formatSet, OGG, VorbisCommentTag.class);
    defaultTagIsInstanceOf(
      formatSet,
      MP3,
      TagOptionSingleton.createDefaultID3Tag().getClass()
    );
    ...
    defaultTagIsInstanceOf(formatSet, OPUS, VorbisCommentTag.class);
    ...
    assertTrue(formatSet.isEmpty(), "Did not test all formats. " + formatSet);
  }

  private void defaultTagIsInstanceOf(
    EnumSet<SupportedFileFormat> formatSet,
    ...
    assertTrue(
      tagClass.isInstance(defaultTag),
      "Expected:" + tagClass + " Actual:" + defaultTag.getClass()
    );
  }
}
