...
public class WmaSimpleTest extends AbstractTestCase {
  ...
  public void testReadFileFromMediaMonkey3() {
    ...
    try {
      ...
      assertEquals(
        "tracktitle",
        tag.getFirst(AsfFieldKey.TITLE.getFieldName())
      );
  ...
  public void testReadFileFromPicardQt() {
    ...
    try {
      ...
      assertEquals(
        "Sonic Youth",
        tag.getFirst(AsfFieldKey.AUTHOR.getFieldName())
      );
      ...
      assertEquals(
        "Sonic Youth",
        tag.getFirst(AsfFieldKey.ALBUM_ARTIST.getFieldName())
      );

      assertEquals("Blast First", tag.getFirst(FieldKey.RECORD_LABEL));
      assertEquals(
        "Blast First",
        tag.getFirst(AsfFieldKey.RECORD_LABEL.getFieldName())
      );

      assertEquals("Sonic Youth", tag.getFirst(FieldKey.ARTIST_SORT));
      assertEquals(
        "Sonic Youth",
        tag.getFirst(AsfFieldKey.ARTIST_SORT.getFieldName())
      );
      ...
      assertEquals(
        "Sonic Youth",
        tag.getFirst(AsfFieldKey.ALBUM_ARTIST_SORT.getFieldName())
      );

      assertEquals(
        "official",
        tag.getFirst(FieldKey.MUSICBRAINZ_RELEASE_STATUS)
      );
      assertEquals(
        "official",
        tag.getFirst(AsfFieldKey.MUSICBRAINZ_RELEASE_STATUS.getFieldName())
      );
      ...
      assertEquals(
        "GB",
        tag.getFirst(AsfFieldKey.MUSICBRAINZ_RELEASE_COUNTRY.getFieldName())
      );
    ...
    try {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        for (FieldKey key : FieldKey.values()) {
          if (
            !(key == FieldKey.COVER_ART) && !(key == FieldKey.ITUNES_GROUPING)
          ) {
  ...
  public void testReadFileWithGifArtwork() {
    ...
    try {
      ...
      assertEquals(
        ...
        coverartField.getImage().getType()
      );
  ...
  public void testReadFileWithJpgArtwork() {
    ...
    try {
      ...
      assertEquals(
        coverartField.getRawImageData().length,
        coverartField.getImageDataSize()
      );
