...
public class OpusVorbisReadTagTest {
    ...
    public void testWriteBigTagToOgg() throws Exception {
        File testFile = AbstractTestCase.copyAudioToTmp("test.opus", new File("test-opus-file-big.opus"));
    ...
    public void testWriteSmallTagToOgg() throws Exception {
        File testFile = AbstractTestCase.copyAudioToTmp("test.opus", new File("test-opus-file-small.opus"));
        AudioFile f = AudioFileIO.read(testFile);
        Tag tag = f.getTag();
