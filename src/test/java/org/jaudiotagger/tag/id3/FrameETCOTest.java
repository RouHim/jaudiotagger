...
public class FrameETCOTest extends AbstractTestCase {
  ...
  public void testReadFile() throws Exception {
    ...
    try {
      ...
      final Map.Entry<Long, int[]> entry = body
        .getTimingCodes()
        .entrySet()
        .iterator()
        .next();
  ...
  public void testSaveToFile() throws Exception {
    ...
    final FrameBodyETCO referenceBody =
      (FrameBodyETCO) referenceFrame.getBody();
    ...
    ID3v24Frame frame = (ID3v24Frame) mp3File
      .getID3v2Tag()
      .getFrame(ID3v24Frames.FRAME_ID_EVENT_TIMING_CODES);
    FrameBodyETCO body = (FrameBodyETCO) frame.getBody();
    assertEquals(referenceBody.getTimestampFormat(), body.getTimestampFormat());

    final Iterator<Map.Entry<Long, int[]>> reference = referenceBody
      .getTimingCodes()
      .entrySet()
      .iterator();
    final Iterator<Map.Entry<Long, int[]>> loaded = body
      .getTimingCodes()
      .entrySet()
      .iterator();
    while (reference.hasNext() && loaded.hasNext()) {
      ...
      assertArrayEquals(refEntry.getValue(), loadedEntry.getValue());
    }
