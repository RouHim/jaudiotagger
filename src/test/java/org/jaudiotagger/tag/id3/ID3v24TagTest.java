...
public class ID3v24TagTest extends AbstractBaseTestCase {
  ...
  public void testCreateID3v24FromID3v11() {
    ...
    assertEquals(
      ID3v11TagTest.TITLE,
      v2Tag.getFirst(ID3v24Frames.FRAME_ID_TITLE)
    );
    assertEquals(
      ID3v11TagTest.YEAR,
  ...
  public void testWriteMultipleTextFields() throws Exception {
    ...
    ID3v24Frame frame = (ID3v24Frame) f
      .getTag()
      .getFirstField(FieldKey.ALBUM_ARTIST_SORT);
    assertEquals(
      "artist1\u0000artist2",
      ((AbstractFrameBodyTextInfo) frame.frameBody).getText()
    );

    //We can get individual values back using longhand as well
    assertEquals(
      "artist1",
      ((AbstractFrameBodyTextInfo) frame.frameBody).getValueAtIndex(0)
    );
    assertEquals(
      "artist2",
      ((AbstractFrameBodyTextInfo) frame.frameBody).getValueAtIndex(1)
    );
  ...
  public void testWriteMultipleFields() throws Exception {
    ...
    f
      .getTag()
      .addField(FieldKey.URL_OFFICIAL_RELEASE_SITE, "http://www,test.org");
    f
      .getTag()
      .addField(FieldKey.URL_OFFICIAL_RELEASE_SITE, "http://www,test.org");
    assertEquals(
      1,
      f.getTag().getFields(FieldKey.URL_OFFICIAL_RELEASE_SITE).size()
    );
    assertEquals(1, f.getTag().getFieldCount());
    assertEquals(1, f.getTag().getFieldCount());
