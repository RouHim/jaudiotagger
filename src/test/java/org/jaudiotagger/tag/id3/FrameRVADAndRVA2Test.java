...
public class FrameRVADAndRVA2Test extends AbstractTestCase {
  ...
  public static String cmp(byte[] a, byte[] b) {
    if (a.length != b.length) {
      return (
        "length of byte arrays differ (" + a.length + "!=" + b.length + ")"
      );
    }
    for (int i = 0; i < a.length; i++) {
      if (a[i] != b[i]) {
        return (
          "byte arrays differ at offset " + i + " (" + a[i] + "!=" + b[i] + ")"
        );
      }
    }
    return null;
  ...
  public void testCreateID3v24Frame() {
    ...
    assertEquals(
      ...
      frame.getBody().getObjectValue(DataTypes.OBJ_DATA)
    );
  }
  ...
  public void testSaveToFile() throws Exception {
    ...
    assertNull(
      cmp(
        FrameBodyRVA2Test.TEST_BYTES,
        (byte[]) body.getObjectValue(DataTypes.OBJ_DATA)
      )
    );
  }
