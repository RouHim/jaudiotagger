...
public class TrunBox extends FullBox {
  ...
  public static Factory copy(TrunBox other) {
    TrunBox box = TrunBox.createTrunBox2(
      other.sampleCount,
      other.dataOffset,
      other.firstSampleFlags,
      other.sampleDuration,
      other.sampleSize,
      other.sampleFlags,
      other.sampleCompositionOffset
    );
  ...
  public static TrunBox createTrunBox2(
    int sampleCount,
    int dataOffset,
  ...
  public static class Factory {
    ...
    public Factory firstSampleFlags(int firstSampleFlags) {
      if (box.isSampleFlagsAvailable()) throw new IllegalStateException(
        "Sample flags already set on this object"
      );
    ...
    public Factory sampleDuration(int[] sampleDuration) {
      if (
        sampleDuration.length != box.sampleCount
      ) throw new IllegalArgumentException(
        "Argument array length not equal to sampleCount"
      );
    ...
    public Factory sampleSize(int[] sampleSize) {
      if (
        sampleSize.length != box.sampleCount
      ) throw new IllegalArgumentException(
        "Argument array length not equal to sampleCount"
      );
    ...
    public Factory sampleFlags(int[] sampleFlags) {
      if (
        sampleFlags.length != box.sampleCount
      ) throw new IllegalArgumentException(
        "Argument array length not equal to sampleCount"
      );
      if (box.isFirstSampleFlagsAvailable()) throw new IllegalStateException(
        "First sample flags already set on this object"
      );
    ...
    public Factory sampleCompositionOffset(int[] sampleCompositionOffset) {
      if (
        sampleCompositionOffset.length != box.sampleCount
      ) throw new IllegalArgumentException(
        "Argument array length not equal to sampleCount"
      );
  ...
  public void parse(ByteBuffer input) {
    ...
    if (
      isSampleFlagsAvailable() && isFirstSampleFlagsAvailable()
    ) throw new RuntimeException("Broken stream");

    sampleCount = input.getInt();
    if (isDataOffsetAvailable()) dataOffset = input.getInt();
    if (isFirstSampleFlagsAvailable()) firstSampleFlags = input.getInt();
    if (isSampleDurationAvailable()) sampleDuration = new int[sampleCount];
    if (isSampleSizeAvailable()) sampleSize = new int[sampleCount];
    if (isSampleFlagsAvailable()) sampleFlags = new int[sampleCount];
    if (isSampleCompositionOffsetAvailable()) sampleCompositionOffset =
      new int[sampleCount];

    for (int i = 0; i < sampleCount; i++) {
      if (isSampleDurationAvailable()) sampleDuration[i] = input.getInt();
      if (isSampleSizeAvailable()) sampleSize[i] = input.getInt();
      if (isSampleFlagsAvailable()) sampleFlags[i] = input.getInt();
      if (isSampleCompositionOffsetAvailable()) sampleCompositionOffset[i] =
        input.getInt();
    }
  }
  ...
  public void doWrite(ByteBuffer out) {
    ...
    if (isDataOffsetAvailable()) out.putInt(dataOffset);
    if (isFirstSampleFlagsAvailable()) out.putInt(firstSampleFlags);

    for (int i = 0; i < sampleCount; i++) {
      if (isSampleDurationAvailable()) out.putInt(sampleDuration[i]);
      if (isSampleSizeAvailable()) out.putInt(sampleSize[i]);
      if (isSampleFlagsAvailable()) out.putInt(sampleFlags[i]);
      if (isSampleCompositionOffsetAvailable()) out.putInt(
        sampleCompositionOffset[i]
      );
    }
  }
