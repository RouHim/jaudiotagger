...
public class MovieHeaderBox extends FullBox {
  ...
  public static MovieHeaderBox createMovieHeaderBox(
    int timescale,
    long duration,
    float rate,
    float volume,
  ...
  private int[] readMatrix(ByteBuffer input) {
    ...
    for (int i = 0; i < 9; i++) matrix[i] = input.getInt();
    return matrix;
  }
  ...
  private void writeMatrix(ByteBuffer out) {
    for (int i = 0; i < Math.min(9, matrix.length); i++) out.putInt(matrix[i]);
    for (int i = Math.min(9, matrix.length); i < 9; i++) out.putInt(0);
  }
