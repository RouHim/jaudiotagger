...
public class MetaBox extends NodeBox {
  ...
  public Map<String, MetaValue> getKeyedMeta() {
    ...
    MdtaBox[] keys = NodeBox.findAllPath(
      this,
      MdtaBox.class,
      new String[] { KeysBox.fourcc(), MdtaBox.fourcc() }
    );

    if (ilst == null || keys.length == 0) return result;
    ...
    for (Entry<Integer, List<Box>> entry : ilst.getValues().entrySet()) {
      ...
      if (db == null) continue;
      MetaValue value = MetaValue.createOtherWithLocale(
        db.getType(),
        db.getLocale(),
        db.getData()
      );
  ...
  public Map<String, MetaValue> getRdnsMeta() {
    ...
    if (
      ilst == null || ilst.getRdnsValues() == null
    ...
    for (ReverseDnsBox rdns : ilst.getRdnsValues()) {
      if (!Utils.isEmpty(rdns.getName()) && !Utils.isEmpty(rdns.getIssuer())) {
        if (rdns.getData() != null && rdns.getData().length > 0) {
          map.put(
            "----:" + rdns.getIssuer() + ":" + rdns.getName(),
            MetaValue.createOtherWithLocale(rdns.getDataBox())
          );
        }
      }
    }
  ...
  public void setKeyedMeta(Map<String, MetaValue> map) {
    ...
    for (Entry<String, MetaValue> entry : map.entrySet()) {
      ...
      children.add(
        DataBox.createDataBox(v.getType(), v.getLocale(), v.getData())
      );
      data.put(i, children);
      ++i;
    }
  ...
  public void setItunesMeta(Map<Integer, MetaValue> map) {
    ...
    if (ilst == null) {
      ...
      for (Entry<Integer, List<Box>> entry : data.entrySet()) {
        ...
        if (v != null) {
          DataBox dataBox = DataBox.createDataBox(
            v.getType(),
            v.getLocale(),
            v.getData()
          );
