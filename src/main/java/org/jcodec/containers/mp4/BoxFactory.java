...
public class BoxFactory implements IBoxFactory {
  ...
  private static final IBoxFactory instance = new BoxFactory(
    new DefaultBoxes()
  );
  ...
  private static final IBoxFactory timecode = new BoxFactory(
    new TimecodeBoxes()
  );
  ...
  public Box newBox(Header header) {
    ...
    if (claz == null) return new Box.LeafBox(header);
