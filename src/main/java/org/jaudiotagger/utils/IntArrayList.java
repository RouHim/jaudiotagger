...
public class IntArrayList {
  ...
  public void set(int index, int value) {
    ...
    if (chunk < chunks.size()) chunks.get(chunk)[off] = value;
    else storage[off] = value;
  }
  ...
  public void addAll(int[] other) {
    ...
    while (otherOff < other.length) {
      ...
      if (copyAmount < 32) {
        for (int i = 0; i < copyAmount; i++) storage[_size++] =
          other[otherOff++];
      } else {
  ...
  public boolean contains(int needle) {
    ...
    for (int i = coff; i < _size; i++) if (storage[i] == needle) return true;
    return false;
  }
}
