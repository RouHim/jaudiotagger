...
public class DefaultMutableTreeNode
  ...
  public void add(MutableTreeNode newChild) {
    if (newChild != null && newChild.getParent() == this) insert(
      newChild,
      getChildCount() - 1
    );
  ...
  public Object[] getUserObjectPath() {
    ...
    for (
      int counter = 0;
      counter < realPath.length;
      counter++
    ) retPath[counter] =
      ((DefaultMutableTreeNode) realPath[counter]).getUserObject();
    return retPath;
  }
  ...
  private void readObject(ObjectInputStream s)
    ...
    if (tValues.length > 0 && tValues[0].equals("userObject")) userObject =
      tValues[1];
  }
