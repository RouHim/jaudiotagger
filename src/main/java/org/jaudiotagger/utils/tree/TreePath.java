...
public class TreePath implements Serializable {
  ...
  public TreePath(Object singlePath) {
    if (singlePath == null) throw new IllegalArgumentException(
  ...
  protected TreePath(TreePath parent, Object lastElement) {
    if (lastElement == null) throw new IllegalArgumentException(
      "path in TreePath must be non null."
    );
    parentPath = parent;
  ...
  public boolean isDescendant(TreePath aTreePath) {
    ...
    if (aTreePath != null) {
      ...
      if (
        oPathLength < pathLength // Can't be a descendant, has fewer components in the path.
      ) return false;
  ...
  public String toString() {
    ...
    for (
      int counter = 0, maxCounter = getPathCount();
      counter < maxCounter;
      counter++
    ) {
  ...
  private void writeObject(ObjectOutputStream s) throws IOException {
    ...
    if (
      lastPathComponent != null && (lastPathComponent instanceof Serializable)
    ) {
