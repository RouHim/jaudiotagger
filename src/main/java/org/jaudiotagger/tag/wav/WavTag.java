...
public class WavTag implements Tag, Id3SupportingTag {
  ...
  private static final Logger logger = LoggerFactory.getLogger(
    WavTag.class.getPackage().getName()
  );
  ...
  public String toString() {
    ...
    if (id3Tag != null) {
      ...
      if (isExistingId3Tag()) {
        sb.append(
          "\tstartLocation:" +
            Hex.asDecAndHex(getStartLocationInFileOfId3Chunk()) +
            "\n"
        );
        sb.append(
          "\tendLocation:" +
            Hex.asDecAndHex(getEndLocationInFileOfId3Chunk()) +
            "\n"
        );
      }
    ...
    try {
      for (FieldKey fieldKey : WavInfoTag.getSupportedKeys()) {
        if (!infoTag.getFirst(fieldKey).isEmpty()) {
          id3Tag.setField(
            fieldKey,
            stripNullTerminator(infoTag.getFirst(fieldKey))
          );
        } else {
