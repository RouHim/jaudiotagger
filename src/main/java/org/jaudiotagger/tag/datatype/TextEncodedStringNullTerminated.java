...
public class TextEncodedStringNullTerminated extends AbstractString {
   ...
   * @param frameBody
   */
  public TextEncodedStringNullTerminated(
    String identifier,
  ...
  public TextEncodedStringNullTerminated(
    TextEncodedStringNullTerminated object
  ) {
    super(object);
  }
  ...
  public void readByteArray(byte[] arr, int offset)
    ...
    logger.debug("Reading from array starting from offset:" + offset);
    int size;
    ...
    while (buffer.hasRemaining()) {
      ...
      if (nextByte == 0x00) {
        if (nullIsOneByte) {
          ...
          logger.debug("Null terminator found starting at:" + endPosition);
          ...
          if (buffer.hasRemaining()) {
            ...
            if (nextByte == 0x00) {
              ...
              logger.debug(
                "UTF16:Null terminator found starting  at:" + endPosition
              );
              isNullTerminatorFound = true;
              break;
            } else {
    ...
    logger.debug("End Position is:" + endPosition + "Offset:" + offset);
    ...
    logger.debug("Text size is:" + bufferSize);
    if (bufferSize == 0) {
      value = "";
    } else {
  ...
  public byte[] writeByteArray() {
    logger.debug("Writing NullTerminatedString." + value);
    byte[] data;
    ...
    try {
      if (StandardCharsets.UTF_16.equals(charset)) {
        if (TagOptionSingleton.getInstance().isEncodeUTF16BomAsLittleEndian()) {
          ...
          final ByteBuffer bb = encoder.encode(
            CharBuffer.wrap('\ufeff' + (String) value + '\0')
          );
    ...
    } catch (CharacterCodingException ce) { //https://bitbucket.org/ijabz/jaudiotagger/issue/1/encoding-metadata-to-utf-16-can-fail-if
  ...
  protected Charset getTextEncodingCharSet() {
    ...
    final Charset charset = TextEncoding.getInstanceOf().getCharsetForId(
      textEncoding
    );
    logger.debug(
