...
public class PartOfSet extends AbstractString {
    ...
    public void readByteArray(byte[] arr, int offset)
        ...
        ByteBuffer inBuffer = ByteBuffer.wrap(
                arr,
                offset,
                arr.length - offset
        ).slice();
    ...
    public byte[] writeByteArray() {
        ...
        } catch (CharacterCodingException ce) { //Should never happen so if does throw a RuntimeException
    ...
    public static class PartOfSetValue {
        ...
        static {
            ...
            trackNoPatternWithTotalCount = Pattern.compile(
                    "([0-9]+)/([0-9]+)(.*)",
                    Pattern.CASE_INSENSITIVE
            );
            trackNoPattern = Pattern.compile(
                    "([0-9]+)(.*)",
                    Pattern.CASE_INSENSITIVE
            );
        }
        ...
        public String getCountAsText() {
            ...
            if (!TagOptionSingleton.getInstance().isPadNumbers()) {
                ...
                padNumber(
                        sb,
                        count,
                        TagOptionSingleton.getInstance().getPadNumberTotalLength()
                );
            }
            return sb.toString();
        }
