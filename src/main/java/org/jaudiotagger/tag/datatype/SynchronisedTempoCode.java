...
public class SynchronisedTempoCode
  ...
  private static final Logger logger = LoggerFactory.getLogger(
    SynchronisedTempoCode.class
  );

  private final TempoCode tempo = new TempoCode(
  ...
  public void setTempo(final int tempo) {
    if (tempo < 0 || tempo > 510) throw new IllegalArgumentException(
      "Tempo must be a positive value less than 511: " + tempo
    );
    this.tempo.setValue(tempo);
  }
  ...
  public String toString() {
    return (
      getTempo() +
      " (\"" +
      EventTimingTypes.getInstanceOf().getValueForId(getTempo()) +
      "\"), " +
      getTimestamp()
    );
  }
