...
public class TextEncodedStringSizeTerminated extends AbstractString {
   ...
   * @param frameBody
   */
  public TextEncodedStringSizeTerminated(
    String identifier,
   ...
   */
  protected ByteBuffer writeString(
   ...
   * @throws CharacterCodingException
   */
  protected ByteBuffer writeStringUTF16LEBOM(
    final String next,
    final int i,
    final int noOfValues
  ) throws CharacterCodingException {
  ...
  protected void checkTrailingNull(List<String> values, String stringValue) {
    if (!TagOptionSingleton.getInstance().isRemoveTrailingTerminatorOnWrite()) {
      if (
        ...
        stringValue.charAt(stringValue.length() - 1) == '\0'
      ) {
