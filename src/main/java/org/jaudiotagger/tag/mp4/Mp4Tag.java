...
public class Mp4Tag extends AbstractTag {
  ...
  private static final EnumMap<FieldKey, Mp4FieldKey> tagFieldToMp4Field =
    new EnumMap<>(FieldKey.class);

  //Mapping from generic key to mp4 key
  static {
    tagFieldToMp4Field.put(
    ...
    tagFieldToMp4Field.put(
      ...
      Mp4FieldKey.MUSICBRAINZ_WORK_COMPOSITION_ID
    );
    ...
    tagFieldToMp4Field.put(
      ...
      Mp4FieldKey.MUSICBRAINZ_WORK_COMPOSITION
    );
    tagFieldToMp4Field.put(
   ...
   */
  public List<TagField> get(Mp4FieldKey mp4FieldKey)
    throws KeyNotFoundException {
    if (mp4FieldKey == null) {
      throw new KeyNotFoundException();
    }
  ...
  public void deleteField(FieldKey genericKey) throws KeyNotFoundException {
    ...
    if (genericKey == FieldKey.KEY) {
      ...
      if (trackTotal.length() == 0) {
        ...
        Mp4TrackField field = (Mp4TrackField) this.getFirstField(
      ...
      if (discTotal.length() == 0) {
        ...
        Mp4DiscNoField field = (Mp4DiscNoField) this.getFirstField(
          FieldKey.DISC_TOTAL
        );
        field.setDiscNo(0);
      }
    ...
    if (
      ...
      try {
        ...
        throw new FieldDataInvalidException(
          "Value " + value + " is not a number as required",
          nfe
        );
      }
    } else if (genericKey == FieldKey.GENRE) {
   ...
   */
  @Override
  public void setField(FieldKey genericKey, String... value)
    throws KeyNotFoundException, FieldDataInvalidException {
    TagField tagfield = createField(genericKey, value);
   ...
   * @throws FieldDataInvalidException
   */
  public void setField(Mp4FieldKey fieldKey, String value)
    throws KeyNotFoundException, FieldDataInvalidException {
  ...
  public void setField(TagField field) {
    ...
    if (field.getId().equals(TRACK.getFieldName())) {
      ...
      if (list == null || list.size() == 0) {
        ...
        Mp4DiscNoField mergedDiscNoField = new Mp4DiscNoField(
          discNo,
          discTotal
        );
   ...
   * @throws FieldDataInvalidException
   */
  public TagField createField(Mp4FieldKey mp4FieldKey, String value)
    throws KeyNotFoundException, FieldDataInvalidException {
    if (value == null) {
      throw new IllegalArgumentException(
        ErrorMessage.GENERAL_INVALID_NULL_ARGUMENT.getMsg()
    ...
    if (mp4FieldKey == Mp4FieldKey.COMPILATION) {
      ...
      mp4FieldKey.getSubClassFieldType() == Mp4TagFieldSubType.DISC_NO
    ) {
      return new Mp4DiscNoField(value);
    } else if (
      mp4FieldKey.getSubClassFieldType() == Mp4TagFieldSubType.TRACK_NO
    ) {
      return new Mp4TrackField(value);
    } else if (mp4FieldKey.getSubClassFieldType() == Mp4TagFieldSubType.BYTE) {
      return new Mp4TagByteField(
        mp4FieldKey,
        value,
        mp4FieldKey.getFieldLength()
      );
    } else if (
      mp4FieldKey.getSubClassFieldType() == Mp4TagFieldSubType.NUMBER
    ) {
      return new Mp4TagTextSingleNumberField(mp4FieldKey.getFieldName(), value);
    } else if (
      mp4FieldKey.getSubClassFieldType() == Mp4TagFieldSubType.REVERSE_DNS
    ) {
      return new Mp4TagReverseDnsField(mp4FieldKey, value);
    } else if (
      mp4FieldKey.getSubClassFieldType() == Mp4TagFieldSubType.ARTWORK
    ) {
      ...
      mp4FieldKey.getSubClassFieldType() == Mp4TagFieldSubType.UNKNOWN
    ) {
      throw new UnsupportedOperationException(
  ...
  public List<Artwork> getArtworkList() {
    ...
    for (TagField next : coverartList) {
      ...
      artwork.setMimeType(
        Mp4TagCoverField.getMimeTypeForImageType(mp4CoverArt.getFieldType())
      );
  ...
  public TagField createCompilationField(boolean origValue)
    throws KeyNotFoundException, FieldDataInvalidException {
    String value = "";
    if (origValue) {
