...
public class ID3v1Iterator implements Iterator {
  ...
  private boolean hasNext(int index) {
    switch (index) {
      ...
      case TRACK:
        if (id3v1tag instanceof ID3v11Tag) {
          return (
            (((ID3v11Tag) id3v1tag).track >= (byte) 0) || hasNext(index + 1)
          );
        }
      default:
        return false;
  ...
  private Object next(int index) {
    switch (lastIndex) {
      ...
      case TITLE:
        return (id3v1tag.artist.length() > 0)
          ? id3v1tag.artist
          : next(index + 1);
      case ARTIST:
        return (id3v1tag.album.length() > 0) ? id3v1tag.album : next(index + 1);
      case ALBUM:
        return (id3v1tag.comment.length() > 0)
          ? id3v1tag.comment
          : next(index + 1);
      case COMMENT:
      ...
      case GENRE:
        return (
            id3v1tag instanceof ID3v11Tag &&
            (((ID3v11Tag) id3v1tag).track >= (byte) 0)
          )
          ? ((ID3v11Tag) id3v1tag).track
          : null;
      default:
