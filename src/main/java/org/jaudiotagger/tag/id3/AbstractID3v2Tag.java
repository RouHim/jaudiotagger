...
public abstract class AbstractID3v2Tag extends AbstractID3Tag implements Tag {
   ...
   * @throws IOException
   */
  private static boolean isID3V2Header(RandomAccessFile raf)
    throws IOException {
  ...
  private static boolean isID3V2Header(FileChannel fc) throws IOException {
    ...
    ByteBuffer headerBuffer = Utils.readFileDataIntoBufferBE(
      fc,
      FIELD_TAGID_LENGTH
    );
    fc.position(start);
  ...
  protected void copyPrimitives(AbstractID3v2Tag copyObject) {
    logger.debug("Copying Primitives");
   ...
   */
  protected abstract List<AbstractID3v2Frame> convertFrame(
  ...
  public boolean hasFrameAndBody(String identifier) {
    if (hasFrame(identifier)) {
      ...
      if (o instanceof AbstractID3v2Frame) {
        return !(
          ((AbstractID3v2Frame) o).getBody() instanceof FrameBodyUnsupported
        );
      }
      return true;
    }
  ...
  public void setField(FieldKey genericKey, String... values)
   ...
   * @param nextFrame
   */
  public void mergeNumberTotalFrames(
    AbstractID3v2Frame newFrame,
    AbstractID3v2Frame nextFrame
  ) {
    AbstractFrameBodyNumberTotal newBody =
      (AbstractFrameBodyNumberTotal) newFrame.getBody();
    AbstractFrameBodyNumberTotal oldBody =
      (AbstractFrameBodyNumberTotal) nextFrame.getBody();
   ...
   * @param frames
   */
  public void mergeDuplicateFrames(
    AbstractID3v2Frame newFrame,
    ...
    for (
      ...
      if (newFrame.getBody() instanceof FrameBodyTXXX) {
        ...
        if (
          ((FrameBodyTXXX) newFrame.getBody()).getDescription().equals(
        ...
        //Value with matching key exists so replace
        if (
          ((FrameBodyWXXX) newFrame.getBody()).getDescription().equals(
        ...
        if (
          ((FrameBodyCOMM) newFrame.getBody()).getDescription().equals(
        ...
        if (
          ((FrameBodyUFID) newFrame.getBody()).getOwner().equals(
            ((FrameBodyUFID) nextFrame.getBody()).getOwner()
          )
        ) {
          li.set(newFrame);
          frameMap.put(newFrame.getId(), frames);
        ...
        if (
          ((FrameBodyUSLT) newFrame.getBody()).getDescription().equals(
        ...
        if (
          ((FrameBodyPOPM) newFrame.getBody()).getEmailToUser().equals(
            ((FrameBodyPOPM) nextFrame.getBody()).getEmailToUser()
          )
        ) {
          li.set(newFrame);
          frameMap.put(newFrame.getId(), frames);
      ...
      //e.g TIPL IPLS, TMCL
        ...
        AbstractFrameBodyPairs existingFrameBody =
          (AbstractFrameBodyPairs) nextFrame.getBody();
   ...
   * @param frame
   */
  private void addNewFrameOrAddField(
    List<TagField> list,
    HashMap frameMap,
    AbstractID3v2Frame existingFrame,
    AbstractID3v2Frame frame
    ...
    if (frame.getBody() instanceof FrameBodyTXXX) {
      ...
      while (i.hasNext()) {
        FrameBodyTXXX existingFrameBody =
          (FrameBodyTXXX) ((AbstractID3v2Frame) i.next()).getBody();
        if (
          frameBody.getDescription().equals(existingFrameBody.getDescription())
        ) {
      ...
      Iterator<TagField> i = mergedList.listIterator();
      while (i.hasNext()) {
        FrameBodyWXXX existingFrameBody =
          (FrameBodyWXXX) ((AbstractID3v2Frame) i.next()).getBody();
        if (
          frameBody.getDescription().equals(existingFrameBody.getDescription())
        ) {
      ...
      AbstractFrameBodyTextInfo frameBody =
        (AbstractFrameBodyTextInfo) frame.getBody();
      AbstractFrameBodyTextInfo existingFrameBody =
        (AbstractFrameBodyTextInfo) existingFrame.getBody();
      existingFrameBody.addTextValue(frameBody.getText());
    } else if (frame.getBody() instanceof AbstractFrameBodyPairs) {
      AbstractFrameBodyPairs frameBody =
        (AbstractFrameBodyPairs) frame.getBody();
      AbstractFrameBodyPairs existingFrameBody =
        (AbstractFrameBodyPairs) existingFrame.getBody();
      existingFrameBody.addPair(frameBody.getText());
    } else if (frame.getBody() instanceof AbstractFrameBodyNumberTotal) {
      AbstractFrameBodyNumberTotal frameBody =
        (AbstractFrameBodyNumberTotal) frame.getBody();
      AbstractFrameBodyNumberTotal existingFrameBody =
        (AbstractFrameBodyNumberTotal) existingFrame.getBody();
  ...
  public void setField(TagField field) throws FieldDataInvalidException {
    if (
      (!(field instanceof AbstractID3v2Frame)) &&
      (!(field instanceof AggregatedFrame))
    ) {
      throw new FieldDataInvalidException(
        "Field " +
          field +
          " is not of type AbstractID3v2Frame nor AggregatedFrame"
      );
    }
  ...
  public void setFrame(String identifier, List<AbstractID3v2Frame> multiFrame) {
    logger.debug("Adding " + multiFrame.size() + " frames for " + identifier);
  ...
  public void removeFrame(String identifier) {
    logger.debug("Removing frame with identifier:" + identifier);
    ...
    for (Iterator i = iterator(); i.hasNext(); ) {
      ...
      if (o instanceof AbstractID3v2Frame) {
        if (
          ...
          logger.debug(
            "Removing frame" + ((AbstractID3v2Frame) o).getIdentifier()
          );
          i.remove();
        }
      }
  ...
  public void removeFrameOfType(String identifier) {
    ...
    for (String match : result) {
      logger.debug(
        "Removing frame with identifier:" +
          match +
          "because starts with:" +
          identifier
      );
  ...
  protected FileLock getFileLockForWriting(
    ...
    logger.debug("locking fileChannel for " + filePath);
    FileLock fileLock;
    try {
      fileLock = fileChannel.tryLock();
    } catch (IOException exception) { //Assumes locking is not supported on this platform so just returns null
      return null;
   ...
   * @throws IOException TODO should be abstract
   */
  public void write(WritableByteChannel channel, int currentTagSize)
    throws IOException {}
   ...
   * @throws IOException
   */
  public void write(OutputStream outputStream, int currentTagSize)
    throws IOException {
   ...
   * @throws IOException
   */
  protected void writePadding(WritableByteChannel channel, int padding)
    throws IOException {
    if (padding > 0) {
  ...
  public void adjustPadding(File file, int paddingSize, long audioStart)
    ...
    logger.debug("Need to move audio file to accommodate tag");
    ...
    } catch (IOException ioe) { //Vista:Can occur if have Write permission on folder this file would be created in Denied
      logger.error(ioe.getMessage(), ioe);
      if (
        ioe.getMessage().equals(FileSystemMessage.ACCESS_IS_DENIED.getMsg())
      ) {
        logger.error(
    ...
    try {
    ...
    } catch (FileNotFoundException ioe) { //Vista:Can occur if have special permission Create Folder/Append Data denied
      logger.error(ioe.getMessage(), ioe);
      logger.error(
      ...
      throw new UnableToModifyFileException(
        ErrorMessage.GENERAL_WRITE_FAILED_TO_MODIFY_TEMPORARY_FILE_IN_FOLDER.getMsg(
          file.getName(),
          file.getParentFile().getPath()
        )
    ...
    try {
      ...
      logger.debug("Copying:" + (file.length() - audioStart) + "bytes");
      ...
      if (audiolength <= MAXIMUM_WRITABLE_CHUNK_SIZE) {
        ...
        logger.debug("Written padding:" + written + " Data:" + written2);
  ...
  protected void writeBufferToFile(
    ...
    if (sizeIncPadding > audioStartLocation) {
      logger.debug("Adjusting Padding");
    ...
    try {
      ...
      if (
        fe.getMessage().contains(FileSystemMessage.ACCESS_IS_DENIED.getMsg()) ||
  ...
  private void replaceFile(File originalFile, File newFile) throws IOException {
    ...
    File originalFileBackup = new File(
    ...
    if (!renameOriginalResult) {
      ...
      newFile.delete();
      throw new UnableToRenameFileException(
        ErrorMessage.GENERAL_WRITE_FAILED_TO_RENAME_ORIGINAL_FILE_TO_BACKUP.getMsg(
          originalFile.getAbsolutePath(),
          originalFileBackup.getName()
        )
   ...
   * @param existingFrame
   */
  protected void processDuplicateFrame(
    AbstractID3v2Frame newFrame,
    AbstractID3v2Frame existingFrame
  ) {
   ...
   * @param newFrame
   */
  protected final void copyFrameIntoMap(
    String id,
    AbstractID3v2Frame newFrame
  ) {
   ...
   * @param next
   */
  protected void loadFrameIntoSpecifiedMap(
    HashMap map,
    String frameId,
    ...
    if (
      ...
      if (map.containsKey(frameId)) {
        ...
        if (o instanceof ArrayList) {
          ArrayList<AbstractID3v2Frame> multiValues = (ArrayList<
            AbstractID3v2Frame
          >) o;
          multiValues.add(next);
          logger.debug("Adding Multi Frame(1)" + frameId);
        } else {
          ...
          logger.debug("Adding Multi Frame(2)" + frameId);
        }
      } else {
        logger.debug("Adding Multi FrameList(3)" + frameId);
    ...
    //If duplicate frame just stores the name of the frame and the number of bytes the frame contains
      ...
      this.duplicateBytes += ((AbstractID3v2Frame) frameMap.get(
          frameId
        )).getSize();
    } else {
      logger.debug("Adding Frame" + frameId);
      map.put(frameId, next);
    }
  }
  ...
  public int getSize() {
    ...
    while (iterator.hasNext()) {
      ...
      if (o instanceof AbstractID3v2Frame) {
        ...
        ArrayList<AbstractID3v2Frame> multiFrames = (ArrayList<
          AbstractID3v2Frame
        >) o;
        for (
          ListIterator<AbstractID3v2Frame> li = multiFrames.listIterator();
          li.hasNext();
   ...
   * @param bodyBuffer
   */
  private void writeFramesToBufferStream(
    Map map,
    ByteArrayOutputStream bodyBuffer
  ) {
    //Sort keys into Preferred Order
    TreeSet<String> sortedWriteOrder = new TreeSet<String>(
  ...
  public List<String> getAll(FieldKey genericKey) throws KeyNotFoundException {
    ...
    if (ID3NumberTotalFields.isNumber(genericKey)) {
      if (fields != null && fields.size() > 0) {
        ...
        values.add(
          ((AbstractFrameBodyNumberTotal) frame.getBody()).getNumberAsText()
        );
      }
      return values;
      ...
      if (fields != null && fields.size() > 0) {
        AbstractID3v2Frame frame = (AbstractID3v2Frame) fields.get(0);
        values.add(
          ((AbstractFrameBodyNumberTotal) frame.getBody()).getTotalAsText()
        );
      }
      return values;
      ...
      if (fields != null && fields.size() > 0) {
        AbstractID3v2Frame frame = (AbstractID3v2Frame) fields.get(0);
        values.add(
          String.valueOf(((FrameBodyPOPM) frame.getBody()).getRating())
        );
      }
      return values;
    } else {
  ...
  public Iterator<TagField> getFields() {
    ...
    final Iterator<Map.Entry<String, Object>> it =
      this.frameMap.entrySet().iterator();

    //Iterator used by hasNext() so doesn't effect next()
    final Iterator<Map.Entry<String, Object>> itHasNext =
      this.frameMap.entrySet().iterator();
    ...
    try {
      while (true) {
        ...
        if (next instanceof AbstractID3v2Frame frame) {
          if (
            (frame.getBody() instanceof AbstractFrameBodyTextInfo frameBody) &&
  ...
  protected TagField doCreateTagField(FrameAndSubId formatKey, String... values)
    ...
    if (frame.getBody() instanceof FrameBodyUFID) {
      ...
      ((FrameBodyUFID) frame.getBody()).setUniqueIdentifier(
      ...
      if (formatKey.getSubId() != null) {
        ...
        if (((FrameBodyCOMM) frame.getBody()).isMediaMonkeyFrame()) {
          ((FrameBodyCOMM) frame.getBody()).setLanguage(
      ...
      if (formatKey.getSubId() != null) {
        ((FrameBodyIPLS) (frame.getBody())).addPair(
          formatKey.getSubId(),
          value
        );
      } else {
      ...
      (frame.getBody() instanceof FrameBodyAPIC) ||
      (frame.getBody() instanceof FrameBodyPIC)
    ) {
    ...
    if (formatKey.getSubId() != null) {
      ...
      while (li.hasNext()) {
        ...
        if (next instanceof FrameBodyTXXX) {
          if (
            ((FrameBodyTXXX) next).getDescription().equals(formatKey.getSubId())
          ) {
            values.addAll((((FrameBodyTXXX) next).getValues()));
          }
        } else if (next instanceof FrameBodyWXXX) {
          if (
            ((FrameBodyWXXX) next).getDescription().equals(formatKey.getSubId())
          ) {
            values.addAll((((FrameBodyWXXX) next).getUrlLinks()));
          }
        } else if (next instanceof FrameBodyCOMM) {
          if (
            ((FrameBodyCOMM) next).getDescription().equals(formatKey.getSubId())
          ) {
          ...
          if (((FrameBodyUFID) next).getOwner().equals(formatKey.getSubId())) {
            if (((FrameBodyUFID) next).getUniqueIdentifier() != null) {
              values.add(
                new String(((FrameBodyUFID) next).getUniqueIdentifier())
              );
            }
          }
          ...
          for (Pair<
            String,
            String
          > entry : ((AbstractFrameBodyPairs) next).getPairing().getMapping()) {
   ...
   * @throws KeyNotFoundException
   */
  protected String doGetValueAtIndex(FrameAndSubId formatKey, int index)
    throws KeyNotFoundException {
  ...
  public TagField createLinkedArtworkField(String url) {
    AbstractID3v2Frame frame = createFrame(
      getFrameAndSubIdFromGenericKey(FieldKey.COVER_ART).getFrameId()
    );
    if (frame.getBody() instanceof FrameBodyAPIC) {
      FrameBodyAPIC body = (FrameBodyAPIC) frame.getBody();
      body.setObjectValue(
        DataTypes.OBJ_PICTURE_DATA,
        url.getBytes(StandardCharsets.ISO_8859_1)
      );
  ...
  private void deleteNumberTotalFrame(
    ...
    if (deleteNumberFieldKey) {
      ...
      if (total.length() == 0) {
        ...
        AbstractID3v2Frame frame = (AbstractID3v2Frame) this.getFrame(
          formatKey.getFrameId()
        );
        AbstractFrameBodyNumberTotal frameBody =
          (AbstractFrameBodyNumberTotal) frame.getBody();
   ...
   * @throws KeyNotFoundException
   */
  protected void doDeleteTagField(FrameAndSubId formatKey)
    throws KeyNotFoundException {
    if (formatKey.getSubId() != null) {
      //Get list of frames that this uses
      List<TagField> list = getFields(formatKey.getFrameId());
      ...
      while (li.hasNext()) {
        ...
        //deleting the frame itself unless now empty
          ...
          PairedTextEncodedStringNullTerminated.ValuePairs pairs =
            ((FrameBodyTIPL) next).getPairing();
          ListIterator<Pair<String, String>> pairIterator = pairs
            .getMapping()
            .listIterator();
  ...
  protected abstract FrameAndSubId getFrameAndSubIdFromGenericKey(
    FieldKey genericKey
  );
