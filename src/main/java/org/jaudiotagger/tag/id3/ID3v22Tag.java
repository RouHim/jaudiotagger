...
public class ID3v22Tag extends AbstractID3v2Tag {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ID3v22Tag.class);
  ...
  public boolean equals(Object obj) {
    ...
    return (
      this.unsynchronization == object.unsynchronization && super.equals(obj)
    );
  }
  ...
  protected List<AbstractID3v2Frame> convertFrame(AbstractID3v2Frame frame)
    ...
    if (
      ...
      if (tmpBody.getYear().length() != 0) {
        ...
        ((AbstractFrameBodyTextInfo) newFrame.getBody()).setText(
          tmpBody.getYear()
        );
        frames.add(newFrame);
      }
  ...
  private void readHeaderFlags(ByteBuffer byteBuffer) {
    ...
    if (compression) {
      logger.debug(
        ErrorMessage.ID3_TAG_COMPRESSED.getMsg(getLoggingFilename())
      );
    }
  ...
  public void read(ByteBuffer byteBuffer) throws TagException {
    ...
    logger.debug(getLoggingFilename() + ":" + "Reading tag from file");
  ...
  public String getFirst(ID3v22FieldKey id3v22FieldKey)
    ...
    FieldKey genericKey = ID3v22Frames.getInstanceOf().getGenericKeyFromId3(
      id3v22FieldKey
    );
    if (genericKey != null) {
      ...
      FrameAndSubId frameAndSubId = new FrameAndSubId(
        null,
        id3v22FieldKey.getFrameId(),
        id3v22FieldKey.getSubId()
      );
  ...
  protected FrameAndSubId getFrameAndSubIdFromGenericKey(FieldKey genericKey) {
    ...
    ID3v22FieldKey id3v22FieldKey =
      ID3v22Frames.getInstanceOf().getId3KeyFromGenericKey(genericKey);
    if (id3v22FieldKey == null) {
  ...
  public List<Artwork> getArtworkList() {
    ...
    for (TagField next : coverartList) {
      FrameBodyPIC coverArt =
        (FrameBodyPIC) ((AbstractID3v2Frame) next).getBody();
      Artwork artwork = ArtworkFactory.getNew();
      artwork.setMimeType(
        ImageFormats.getMimeTypeForFormat(coverArt.getFormatType())
      );
  ...
  public TagField createField(Artwork artwork)
    ...
    if (!artwork.isLinked()) {
      ...
      body.setObjectValue(
        ...
        ImageFormats.getFormatForMimeType(artwork.getMimeType())
      );
  ...
  public TagField createArtworkField(byte[] data, String mimeType) {
    ...
    body.setObjectValue(
      DataTypes.OBJ_IMAGE_FORMAT,
      ImageFormats.getFormatForMimeType(mimeType)
    );
    body.setObjectValue(DataTypes.OBJ_DESCRIPTION, "");
    return frame;
  }
