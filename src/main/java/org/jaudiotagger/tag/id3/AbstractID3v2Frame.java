...
public abstract class AbstractID3v2Frame
  ...
  public AbstractID3v2Frame(String identifier) {
    logger.debug("Creating empty frame of type" + identifier);
    this.identifier = identifier;

    // Use reflection to map id to frame body, which makes things much easier
    // to keep things up to date.
    try {
      Class<AbstractID3v2FrameBody> c = (Class<
        AbstractID3v2FrameBody
      >) Class.forName(
        "org.jaudiotagger.tag.id3.framebody.FrameBody" + identifier
      );
      frameBody = c.newInstance();
    } catch (ClassNotFoundException cnfe) {
      logger.error(cnfe.getMessage());
      frameBody = new FrameBodyUnsupported(identifier);
    } catch (InstantiationException ie) { //Instantiate Interface/Abstract should not happen
      logger.error("InstantiationException:" + identifier, ie);
      throw new RuntimeException(ie);
    } catch (IllegalAccessException iae) { //Private Constructor shouild not happen
    ...
    if (this instanceof ID3v24Frame) {
      frameBody.setTextEncoding(
        TagOptionSingleton.getInstance().getId3v24DefaultTextEncoding()
      );
    } else if (this instanceof ID3v23Frame) {
      frameBody.setTextEncoding(
        TagOptionSingleton.getInstance().getId3v23DefaultTextEncoding()
      );
    }

    logger.debug("Created empty frame of type" + identifier);
  }
   ...
   */
  protected AbstractID3v2FrameBody readEncryptedBody(
    String identifier,
    ByteBuffer byteBuffer,
    int frameSize
  ) throws InvalidDataTypeException {
    try {
      AbstractID3v2FrameBody frameBody = new FrameBodyEncrypted(
        identifier,
        byteBuffer,
        frameSize
      );
  ...
  protected AbstractID3v2FrameBody readBody(
    String identifier,
    ByteBuffer byteBuffer,
    int frameSize
    ...
    logger.debug("Creating framebody:start");
    ...
    try {
      ...
      Constructor<AbstractID3v2FrameBody> construct = c.getConstructor(
        constructorParameterTypes
      );
      frameBody = (construct.newInstance(constructorParameterValues));
    } catch (ClassNotFoundException cex) { //No class defined for this frame type,use FrameUnsupported
      logger.debug(
        getLoggingFilename() +
          ":" +
      ...
      logger.error(
        getLoggingFilename() +
          ":" +
          "An error occurred within abstractID3v2FrameBody for identifier:" +
          identifier +
          ":" +
    ...
    } catch (NoSuchMethodException sme) { //No Such Method should not happen
      logger.error(
        getLoggingFilename() + ":" + "No such method:" + sme.getMessage(),
        sme
      );
    ...
    logger.debug(
      getLoggingFilename() +
        ":" +
        "Created framebody:end" +
        frameBody.getIdentifier()
    );
    frameBody.setHeader(this);
    return frameBody;
  }
   ...
   */
  protected String readIdentifier(ByteBuffer byteBuffer)
    throws PaddingException, InvalidFrameException {
    ...
    if ((getFrameHeaderSize() - getFrameIdSize()) > byteBuffer.remaining()) {
      logger.warn(
        getLoggingFilename() + ":" + "No space to find another frame:"
      );
    ...
    logger.debug(getLoggingFilename() + ":" + "Identifier is" + identifier);
    return identifier;
  }
  ...
  protected AbstractID3v2FrameBody readBody(
    ...
    try {
    ...
    } catch (NoSuchMethodException sme) { //If suitable constructor does not exist
      ...
      logger.error(
        "Invocation target exception:" + ite.getCause().getMessage(),
        ite.getCause()
      );
      if (ite.getCause() instanceof Error) {
        throw (Error) ite.getCause();
    ...
    logger.debug("frame Body created" + frameBody.getIdentifier());
    frameBody.setHeader(this);
    return frameBody;
  }
