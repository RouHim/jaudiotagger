...
public class ID3Unsynchronization {
  ...
  public static boolean requiresUnsynchronization(byte[] abySource) {
    for (int i = 0; i < abySource.length - 1; i++) {
      if (
        ((abySource[i] & MPEGFrameHeader.SYNC_BYTE1) ==
          MPEGFrameHeader.SYNC_BYTE1) &&
        ((abySource[i + 1] & MPEGFrameHeader.SYNC_BYTE2) ==
          MPEGFrameHeader.SYNC_BYTE2)
      ) {
        if (logger.isDebugEnabled()) {
          logger.debug("Unsynchronisation required found bit at:" + i);
        }
        return true;
      }
    }
  ...
  public static byte[] unsynchronize(byte[] abySource) {
    ...
    while (input.available() > 0) {
      ...
      if (
        (firstByte & MPEGFrameHeader.SYNC_BYTE1) == MPEGFrameHeader.SYNC_BYTE1
      ) {
        ...
        if (input.available() > 0) {
          ...
          if (
            (secondByte & MPEGFrameHeader.SYNC_BYTE2) ==
            MPEGFrameHeader.SYNC_BYTE2
          ) {
            // we need to unsynchronize here
            if (logger.isDebugEnabled()) {
              logger.debug("Writing unsynchronisation bit at:" + count);
            }
            output.write(0);
          } else if (secondByte == 0) {
            // we need to unsynchronize here
            if (logger.isDebugEnabled()) {
              logger.debug("Inserting zero unsynchronisation bit at:" + count);
            }
            output.write(0);
          }
          input.reset();
    ...
    if (
      (abySource[abySource.length - 1] & MPEGFrameHeader.SYNC_BYTE1) ==
      MPEGFrameHeader.SYNC_BYTE1
    ) {
      logger.debug("Adding unsynchronisation bit at end of stream");
      output.write(0);
    }
  ...
  public static ByteBuffer synchronize(ByteBuffer source) {
    ...
    while (from < len) {
      ...
      copy = ((byteValue & MPEGFrameHeader.SYNC_BYTE1) !=
        MPEGFrameHeader.SYNC_BYTE1);
    }
