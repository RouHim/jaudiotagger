...
public class ID3v23Tag extends AbstractID3v2Tag {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ID3v23Tag.class);
  ...
  protected static int TAG_EXT_HEADER_DATA_LENGTH =
    TAG_EXT_HEADER_LENGTH - FIELD_TAG_EXT_SIZE_LENGTH;
  ...
  protected void processDuplicateFrame(
    ...
    if (
      newFrame.getIdentifier().equals(ID3v23Frames.FRAME_ID_V3_INVOLVED_PEOPLE)
    ) {
      PairedTextEncodedStringNullTerminated.ValuePairs oldVps =
        ((FrameBodyIPLS) (existingFrame).getBody()).getPairing();
      PairedTextEncodedStringNullTerminated.ValuePairs newVps =
        ((FrameBodyIPLS) newFrame.getBody()).getPairing();
  ...
  @Override
  protected List<AbstractID3v2Frame> convertFrame(AbstractID3v2Frame frame)
    throws InvalidFrameException {
    List<AbstractID3v2Frame> frames = new ArrayList<>();
    if (
      (frame.getIdentifier().equals(ID3v24Frames.FRAME_ID_YEAR)) &&
      (frame.getBody() instanceof FrameBodyTDRC tmpBody)
    ) {
      ...
      if (!tmpBody.getDate().equals("")) {
        ...
        ((FrameBodyTDAT) newFrame.getBody()).setMonthOnly(
          tmpBody.isMonthOnly()
        );
        frames.add(newFrame);
      }
      if (!tmpBody.getTime().equals("")) {
        ...
        ((FrameBodyTIME) newFrame.getBody()).setHoursOnly(
          tmpBody.isHoursOnly()
        );
        frames.add(newFrame);
      }
    ...
    //If at later stage we have multiple IPLS frames we have to merge
      ...
      (frame.getIdentifier().equals(ID3v24Frames.FRAME_ID_INVOLVED_PEOPLE)) &&
      (frame.getBody() instanceof FrameBodyTIPL)
    ) {
      List<Pair<String, String>> pairs =
        ((FrameBodyTIPL) frame.getBody()).getPairing().getMapping();
      AbstractID3v2Frame ipls = new ID3v23Frame(
        (ID3v24Frame) frame,
        ID3v23Frames.FRAME_ID_V3_INVOLVED_PEOPLE
      );
      FrameBodyIPLS iplsBody = new FrameBodyIPLS(
        frame.getBody().getTextEncoding(),
        pairs
      );
      ipls.setBody(iplsBody);
      frames.add(ipls);
    } else if (
      (frame.getIdentifier().equals(ID3v24Frames.FRAME_ID_MUSICIAN_CREDITS)) &&
      (frame.getBody() instanceof FrameBodyTMCL)
    ) {
      List<Pair<String, String>> pairs =
        ((FrameBodyTMCL) frame.getBody()).getPairing().getMapping();
      AbstractID3v2Frame ipls = new ID3v23Frame(
        (ID3v24Frame) frame,
        ID3v23Frames.FRAME_ID_V3_INVOLVED_PEOPLE
      );
  ...
  public ID3v23Tag(ID3v23Tag copyObject) {
    ...
    super(copyObject);
    logger.debug("Creating tag from another tag of same type");
  ...
  public ID3v23Tag(AbstractTag mp3tag) {
    logger.debug("Creating tag from a tag of a different version");
    frameMap = new LinkedHashMap();
    encryptedFrameMap = new LinkedHashMap();
    ...
    if (mp3tag != null) {
      ...
      logger.debug("Created tag from a tag of a different version");
    }
  }
  ...
  private void readHeaderFlags(ByteBuffer buffer) {
    ...
    if (isUnsynchronization()) {
      logger.debug(
        ErrorMessage.ID3_TAG_UNSYNCHRONIZED.getMsg(getLoggingFilename())
      );
    }

    if (extended) {
      logger.debug(ErrorMessage.ID3_TAG_EXTENDED.getMsg(getLoggingFilename()));
    }

    if (experimental) {
      logger.debug(
        ErrorMessage.ID3_TAG_EXPERIMENTAL.getMsg(getLoggingFilename())
      );
    }
  }
  ...
  private void readExtendedHeader(ByteBuffer buffer, int size) {
    ...
    if (extendedHeaderSize == TAG_EXT_HEADER_DATA_LENGTH) {
      ...
      logger.debug(ErrorMessage.ID3_TAG_CRC.getMsg(getLoggingFilename()));
      ...
      crc32 = buffer.getInt();
      logger.debug(
        ErrorMessage.ID3_TAG_CRC_SIZE.getMsg(getLoggingFilename(), crc32)
      );
    }
  ...
  public void read(ByteBuffer buffer) throws TagException {
    ...
    logger.debug(getLoggingFilename() + ":" + "Reading ID3v23 tag");
    ...
    logger.debug(ErrorMessage.ID_TAG_SIZE.getMsg(getLoggingFilename(), size));
  ...
  protected void readFrames(ByteBuffer byteBuffer, int size) {
    ...
    while (byteBuffer.position() < size) {
      ...
      try {
        ...
        logger.debug(
          getLoggingFilename() + ":Looking for next frame at:" + posBeforeRead
        );
      ...
      } catch (PaddingException ex) { //Found Padding, no more frames
        logger.debug(
          getLoggingFilename() +
        ...
        break;
      } catch (EmptyFrameException ex) { //Found Empty Frame, log it - empty frames should not exist
        ...
        logger.warn(
          getLoggingFilename() + ":Invalid Frame:" + ife.getMessage()
        );
        this.invalidFrames++;
        //Don't try and find any more frames
        break;
        ...
        logger.warn(
          getLoggingFilename() + ":Corrupt Frame:" + idete.getMessage()
        );
        this.invalidFrames++;
        continue;
      }
  ...
  public long write(File file, long audioStartLocation) throws IOException {
    ...
    logger.debug("Writing tag to file:" + getLoggingFilename());
    ...
    logger.debug(
      getLoggingFilename() +
        ...
        bodyByteBuffer.length
    );

    // Unsynchronize if option enabled and unsync required
    unsynchronization =
      TagOptionSingleton.getInstance().isUnsyncTags() &&
    ...
    int sizeIncPadding = calculateTagSize(
      bodyByteBuffer.length + TAG_HEADER_LENGTH,
      (int) audioStartLocation
    );
    int padding = sizeIncPadding - (bodyByteBuffer.length + TAG_HEADER_LENGTH);
    logger.debug(
      getLoggingFilename() + ":Current audiostart:" + audioStartLocation
    );
    logger.debug(
      getLoggingFilename() + ":Size including padding:" + sizeIncPadding
    );
    logger.debug(getLoggingFilename() + ":Padding:" + padding);

    ByteBuffer headerBuffer = writeHeaderToBuffer(
      padding,
      bodyByteBuffer.length
    );
    writeBufferToFile(
  ...
  public void write(WritableByteChannel channel, int currentTagSize)
    throws IOException {
    logger.debug(getLoggingFilename() + ":Writing tag to channel");
  ...
  public void createStructure() {
    MP3File.getStructureFormatter().openHeadingElement(
      TYPE_TAG,
      getIdentifier()
    );
  ...
  public TagField createField(ID3v23FieldKey id3Key, String value)
    ...
    return super.doCreateTagField(
      new FrameAndSubId(null, id3Key.getFrameId(), id3Key.getSubId()),
      value
    );
  }
  ...
  public String getFirst(ID3v23FieldKey id3v23FieldKey)
    ...
    FieldKey genericKey = ID3v23Frames.getInstanceOf().getGenericKeyFromId3(
      id3v23FieldKey
    );
    if (genericKey != null) {
      return super.getFirst(genericKey);
    } else {
      FrameAndSubId frameAndSubId = new FrameAndSubId(
        null,
  ...
  protected FrameAndSubId getFrameAndSubIdFromGenericKey(FieldKey genericKey) {
    ...
    ID3v23FieldKey id3v23FieldKey =
      ID3v23Frames.getInstanceOf().getId3KeyFromGenericKey(genericKey);
    if (id3v23FieldKey == null) {
      throw new KeyNotFoundException(genericKey.name());
    }
    return new FrameAndSubId(
      genericKey,
      id3v23FieldKey.getFrameId(),
      id3v23FieldKey.getSubId()
    );
  ...
  public List<Artwork> getArtworkList() {
    ...
    for (TagField next : coverartList) {
      FrameBodyAPIC coverArt =
        (FrameBodyAPIC) ((AbstractID3v2Frame) next).getBody();
  ...
  public TagField createField(FieldKey genericKey, String... values)
    ...
    if (genericKey == FieldKey.GENRE) {
      ...
      if (value.length() == 1) {
        ...
        AbstractID3v2Frame tyer = createFrame(ID3v23Frames.FRAME_ID_V3_TYER);
        ((AbstractFrameBodyTextInfo) tyer.getBody()).setText(
          value.substring(0, 4)
        );
  ...
  public String getValue(FieldKey genericKey, int index)
    ...
    if (genericKey == FieldKey.YEAR) {
      AggregatedFrame af = (AggregatedFrame) getFrame(
        TyerTdatAggregatedFrame.ID_TYER_TDAT
      );
      if (af != null) {
  ...
  protected void loadFrameIntoSpecifiedMap(
    ...
    if (
      !(frameId.equals(ID3v23Frames.FRAME_ID_V3_TYER)) &&
      !(frameId.equals(ID3v23Frames.FRAME_ID_V3_TDAT))
    ) {
    ...
    if (frameId.equals(ID3v23Frames.FRAME_ID_V3_TDAT)) {
      if (frame.getContent().length() == 0) {
        ...
        logger.warn("TDAT is empty so just ignoring");
        return;
      }
    }
    if (
      map.containsKey(frameId) ||
      ...
      if (map.containsKey(ID3v23Frames.FRAME_ID_V3_TDAT)) {
        ...
        ag.addFrame(frame);
        ag.addFrame(
          (AbstractID3v2Frame) map.get(ID3v23Frames.FRAME_ID_V3_TDAT)
        );
      ...
      if (map.containsKey(ID3v23Frames.FRAME_ID_V3_TYER)) {
        TyerTdatAggregatedFrame ag = new TyerTdatAggregatedFrame();
        ag.addFrame(
          (AbstractID3v2Frame) map.get(ID3v23Frames.FRAME_ID_V3_TYER)
        );
        ag.addFrame(frame);
