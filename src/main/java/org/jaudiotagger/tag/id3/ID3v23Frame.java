...
public class ID3v23Frame extends AbstractID3v2Frame {
  ...
  protected ID3v23Frame(ID3v24Frame frame, String identifier) {
    ...
    statusFlags = new StatusFlags(
      (ID3v24Frame.StatusFlags) frame.getStatusFlags()
    );
    encodingFlags = new EncodingFlags(frame.getEncodingFlags().getFlags());
  }
    ...
    if (frame instanceof ID3v24Frame) {
      ...
      // Deprecated frame for v24
        ...
        if (ID3Tags.isID3v23FrameIdentifier(frame.getIdentifier())) {
          this.frameBody =
            ((FrameBodyDeprecated) frame.getBody()).getOriginalFrameBody();
          this.frameBody.setHeader(this);
          this.frameBody.setTextEncoding(
            ID3TextEncodingConversion.getTextEncoding(
              this,
              this.frameBody.getTextEncoding()
            )
          );
          identifier = frame.getIdentifier();
          logger.debug(
        ...
        logger.debug("isID3v24FrameIdentifier");
  ...
  public void read(ByteBuffer byteBuffer)
    ...
    if (!isValidID3v2FrameIdentifier(identifier)) {
      logger.debug(getLoggingFilename() + ":Invalid identifier:" + identifier);
    ...
    if (((EncodingFlags) encodingFlags).isNonStandardFlags()) {
      ...
      logger.error(
        getLoggingFilename() +
          ...
          Hex.asHex(encodingFlags.getFlags())
      );
    }
  ...
  public void write(ByteArrayOutputStream tagBuffer) {
    logger.debug("Writing frame to buffer:" + getIdentifier());
  ...
  class StatusFlags extends AbstractID3v2Frame.StatusFlags {
    ...
    private byte convertV4ToV3Flags(byte v4Flag) {
      byte v3Flag = (byte) 0;
      if (
        (v4Flag & ID3v24Frame.StatusFlags.MASK_FILE_ALTER_PRESERVATION) != 0
      ) {
