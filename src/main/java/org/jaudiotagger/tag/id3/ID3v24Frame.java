...
public class ID3v24Frame extends AbstractID3v2Frame {
    private static final Logger logger = LoggerFactory.getLogger(ID3v24Frame.class);
    ...
    private void createV24FrameFromV23Frame(ID3v23Frame frame) throws InvalidFrameException {
        ...
        logger.debug("Creating V24frame from v23:" + frame.getIdentifier() + ":" + identifier);
        ...
        if (frame.getBody() instanceof FrameBodyUnsupported) {
            ...
            logger.debug("V3:UnsupportedBody:Orig id is:" + frame.getIdentifier() + ":New id is:" + identifier);
        }//Simple Copy
        ...
        // Is it a known v3 frame which needs forcing to v4 frame e.g. TYER - TDRC
            ...
            // as a deprecated frame consisting of an array of bytes*/
                ...
                this.frameBody.setHeader(this);
                identifier = frame.getIdentifier();
                logger.debug("V3:Deprecated:Orig id is:" + frame.getIdentifier() + ":New id is:" + identifier);
            }
        }
        // Unknown Frame e.g NCON or TDRL (because TDRL unknown to V23)
            ...
            this.frameBody.setHeader(this);
            identifier = frame.getIdentifier();
            logger.debug("V3:Unknown:Orig id is:" + frame.getIdentifier() + ":New id is:" + identifier);
        }
    }
    ...
    public void read(ByteBuffer byteBuffer) throws InvalidFrameException, InvalidDataTypeException {
        ...
        if (((EncodingFlags) encodingFlags).isDataLengthIndicator()) {
            ...
            logger.debug(getLoggingFilename() + ":" + "Frame Size Is:" + frameSize + " Data Length Size:" + dataLengthSize);
        }
        ...
        if (((EncodingFlags) encodingFlags).isUnsynchronised()) {
            ...
            logger.debug(getLoggingFilename() + ":" + "Frame Size After Syncing is:" + syncSize);
        }
        ...
        try {
            ...
            if (!(frameBody instanceof ID3v24FrameBody)) {
                logger.debug(getLoggingFilename() + ":" + "Converted frame body with:" + identifier + " to deprecated framebody");
    ...
    public void write(ByteArrayOutputStream tagBuffer) {
        ...
        if (unsynchronization) {
            ...
            logger.debug("bodybytebuffer:sizeafterunsynchronisation:" + bodyBuffer.length);
        }
    ...
    class EncodingFlags extends AbstractID3v2Frame.EncodingFlags {
        ...
        public void logEnabledFlags() {
            ...
            if (isGrouping()) {
                logger.debug(ErrorMessage.MP3_FRAME_IS_GROUPED.getMsg(getLoggingFilename(), identifier));
            }

            if (isUnsynchronised()) {
                logger.debug(ErrorMessage.MP3_FRAME_IS_UNSYNCHRONISED.getMsg(getLoggingFilename(), identifier));
            }

            if (isDataLengthIndicator()) {
                logger.debug(ErrorMessage.MP3_FRAME_IS_DATA_LENGTH_INDICATOR.getMsg(getLoggingFilename(), identifier));
            }
        }
