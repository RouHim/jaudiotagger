...
public class ID3v24Frame extends AbstractID3v2Frame {
  ...
  private static final Pattern validFrameIdentifier = Pattern.compile(
    "[A-Z][0-9A-Z]{3}"
  );
  ...
  protected static final int FRAME_HEADER_SIZE =
    FRAME_ID_SIZE + FRAME_SIZE_SIZE + FRAME_FLAGS_SIZE;
  ...
  private void createV24FrameFromV23Frame(ID3v23Frame frame)
    ...
    logger.debug(
      "Creating V24frame from v23:" + frame.getIdentifier() + ":" + identifier
    );

    //We cant convert unsupported bodies properly
    if (frame.getBody() instanceof FrameBodyUnsupported) {
      this.frameBody = new FrameBodyUnsupported(
        (FrameBodyUnsupported) frame.getBody()
      );
      this.frameBody.setHeader(this);
      identifier = frame.getIdentifier();
      logger.debug(
        "V3:UnsupportedBody:Orig id is:" +
          frame.getIdentifier() +
          ":New id is:" +
          identifier
      );
    ...
    //Simple Copy
      ...
      if (
        (frame
            .getIdentifier()
            .equals(ID3v23Frames.FRAME_ID_V3_USER_DEFINED_INFO)) &&
        (((FrameBodyTXXX) frame.getBody()).getDescription().equals(
            FrameBodyTXXX.MOOD
          ))
      ) {
        ...
        logger.debug(
          "V3:Orig id is:" + frame.getIdentifier() + ":New id is:" + identifier
        );
        this.frameBody = (AbstractTagFrameBody) ID3Tags.copyObject(
          frame.getBody()
        );
        this.frameBody.setHeader(this);
      }
    ...
    // Is it a known v3 frame which needs forcing to v4 frame e.g. TYER - TDRC
      ...
      if (identifier != null) {
        ...
        this.frameBody = this.readBody(
          identifier,
          (AbstractID3v2FrameBody) frame.getBody()
        );
        this.frameBody.setHeader(this);
      }
      ...
      // as a deprecated frame consisting of an array of bytes*/
        ...
        this.frameBody = new FrameBodyDeprecated(
          (AbstractID3v2FrameBody) frame.getBody()
        );
        this.frameBody.setHeader(this);
        identifier = frame.getIdentifier();
        logger.debug(
          "V3:Deprecated:Orig id is:" +
            frame.getIdentifier() +
            ":New id is:" +
            identifier
        );
  ...
  protected ID3v24Frame(ID3v23Frame frame, String identifier) {
    ...
    statusFlags = new StatusFlags(
      (ID3v23Frame.StatusFlags) frame.getStatusFlags()
    );
    encodingFlags = new EncodingFlags(frame.getEncodingFlags().getFlags());
  }
  ...
  private void checkIfFrameSizeThatIsNotSyncSafe(ByteBuffer byteBuffer)
    ...
    if (frameSize > ID3SyncSafeInteger.MAX_SAFE_SIZE) {
      ...
      byteBuffer.position(currentPosition - getFrameIdSize());
      boolean isNotSyncSafe = ID3SyncSafeInteger.isBufferNotSyncSafe(
        byteBuffer
      );
      ...
      if (isNotSyncSafe) {
        logger.warn(
          getLoggingFilename() +
            ":" +
            "Frame size is NOT stored as a sync safe integer:" +
            identifier
        );
  ...
  private void getFrameSize(ByteBuffer byteBuffer)
    ...
    if (frameSize < 0) {
      logger.warn(
        getLoggingFilename() + ":" + "Invalid Frame size:" + identifier
      );
      throw new InvalidFrameException(identifier + " is invalid frame");
    } else if (frameSize == 0) {
  ...
  public void read(ByteBuffer byteBuffer)
    ...
    if (!isValidID3v2FrameIdentifier(identifier)) {
      ...
      logger.debug(
        getLoggingFilename() + ":" + "Invalid identifier:" + identifier
      );
    ...
    if (((EncodingFlags) encodingFlags).isUnsynchronised()) {
      ...
      logger.debug(
        getLoggingFilename() + ":" + "Frame Size After Syncing is:" + syncSize
      );
    }
  ...
  public void write(ByteArrayOutputStream tagBuffer) {
    ...
    logger.debug("Writing frame to file:" + getIdentifier());
    ...
    if (unsynchronization) {
      ...
      logger.debug(
        "bodybytebuffer:sizeafterunsynchronisation:" + bodyBuffer.length
      );
    }
    ...
    headerBuffer.put(
      getIdentifier().getBytes(StandardCharsets.ISO_8859_1),
      0,
      FRAME_ID_SIZE
    );
    ...
    logger.debug("Frame Size Is:" + size);
  ...
  public class StatusFlags extends AbstractID3v2Frame.StatusFlags {
    ...
    private byte convertV3ToV4Flags(byte v3Flag) {
      byte v4Flag = (byte) 0;
      if (
        (v3Flag & ID3v23Frame.StatusFlags.MASK_FILE_ALTER_PRESERVATION) != 0
      ) {
    ...
    public void createStructure() {
      ...
      MP3File.getStructureFormatter().addElement(
        ...
        originalFlags & MASK_TAG_ALTER_PRESERVATION
      );
      MP3File.getStructureFormatter().addElement(
        TYPE_FILEALTERPRESERVATION,
        originalFlags & MASK_FILE_ALTER_PRESERVATION
      );
      MP3File.getStructureFormatter().addElement(
        TYPE_READONLY,
        originalFlags & MASK_READ_ONLY
      );
  ...
  class EncodingFlags extends AbstractID3v2Frame.EncodingFlags {
    ...
    public static final String TYPE_FRAMEUNSYNCHRONIZATION =
      ...
      if (isNonStandardFlags()) {
        logger.warn(
          getLoggingFilename() +
            ":" +
            getIdentifier() +
            ":Unsetting Unknown Encoding Flags:" +
            Hex.asHex(flags)
        );
