...
public class ID3v24Tag extends AbstractID3v2Tag {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ID3v24Tag.class);
  ...
  public static final int MASK_V24_TAG_SIZE_RESTRICTIONS =
    (byte) FileConstants.BIT7 | FileConstants.BIT6;
  ...
  public static final int MASK_V24_TEXT_FIELD_SIZE_RESTRICTIONS =
    FileConstants.BIT4 | FileConstants.BIT3;
  ...
  public static final int MASK_V24_IMAGE_SIZE_RESTRICTIONS =
    FileConstants.BIT2 | FileConstants.BIT1;
  ...
  protected List<AbstractID3v2Frame> convertFrame(AbstractID3v2Frame frame)
    ...
    if (
      frame instanceof ID3v22Frame &&
      frame.getIdentifier().equals(ID3v22Frames.FRAME_ID_V2_IPLS)
    ) {
    ...
    if (
      frame instanceof ID3v23Frame &&
      frame.getIdentifier().equals(ID3v23Frames.FRAME_ID_V3_INVOLVED_PEOPLE)
    ) {
      List<Pair<String, String>> pairs =
        ((FrameBodyIPLS) frame.getBody()).getPairing().getMapping();
      ...
      AbstractID3v2Frame tipl = new ID3v24Frame(
        (ID3v23Frame) frame,
        ID3v24Frames.FRAME_ID_INVOLVED_PEOPLE
      );
      FrameBodyTIPL tiplBody = new FrameBodyTIPL(
      ...
      AbstractID3v2Frame tmcl = new ID3v24Frame(
        (ID3v23Frame) frame,
        ID3v24Frames.FRAME_ID_MUSICIAN_CREDITS
      );
      FrameBodyTMCL tmclBody = new FrameBodyTMCL(
        frame.getBody().getTextEncoding(),
        pairsTmcl
      );
    ...
    if (newFrame.getBody() instanceof FrameBodyTDRC) {
      if (existingFrame.getBody() instanceof FrameBodyTDRC body) {
        ...
        if (newBody.getOriginalID().equals(ID3v23Frames.FRAME_ID_V3_TYER)) {
          ...
          newBody.getOriginalID().equals(ID3v23Frames.FRAME_ID_V3_TDAT)
        ) {
          body.setDate(newBody.getDate());
          body.setMonthOnly(newBody.isMonthOnly());
        } else if (
          newBody.getOriginalID().equals(ID3v23Frames.FRAME_ID_V3_TIME)
        ) {
      ...
      // valid frame such as TYER which has been converted to TDRC, replace the firstframe with this frame
        ...
        logger.warn(
          "Found duplicate TDRC frame in invalid situation,discarding:" +
            newFrame.getIdentifier()
        );
      }
    } else {
  ...
  public ID3v24Tag(AbstractTag mp3tag) {
    ...
    if (mp3tag != null) {
       ...
       */
        ...
        this.setLoggingFilename(
          ((AbstractID3v2Tag) mp3tag).getLoggingFilename()
        );
      ...
      //IDv1
        ...
        if (
          ...
          ((id3tag.genre & ID3v1Tag.BYTE_TO_UNSIGNED) !=
          ...
          String genre =
            ...
            genreId +
            ") " +
            GenreTypes.getInstanceOf().getValueForId(genreId);
   ...
   */
  private void readExtendedHeader(ByteBuffer byteBuffer, int size)
  ...
  public void read(ByteBuffer byteBuffer) throws TagException {
    ...
    if (!seek(byteBuffer)) {
      throw new TagNotFoundException(
        getLoggingFilename() + ":" + getIdentifier() + " tag not found"
      );
    }
    logger.debug(getLoggingFilename() + ":" + "Reading ID3v24 tag");
  ...
  protected void readFrames(ByteBuffer byteBuffer, int size) {
    ...
    while (byteBuffer.position() <= size) {
      ...
      try {
        ...
        break;
      } catch (EmptyFrameException ex) { //Found Empty Frame
        logger.warn(
          getLoggingFilename() + ":" + "Empty Frame:" + ex.getMessage()
        );
        ...
        break;
      } catch (InvalidFrameException ife) { //Problem trying to find frame
        logger.warn(
  ...
  public void write(WritableByteChannel channel, int currentTagSize)
    throws IOException {
    logger.error("Writing tag to channel");
   ...
   */
  public TagField createField(ID3v24FieldKey id3Key, String value)
  ...
  public String getFirst(ID3v24FieldKey id3v24FieldKey)
    ...
    FieldKey genericKey = ID3v24Frames.getInstanceOf().getGenericKeyFromId3(
      id3v24FieldKey
    );
    if (genericKey != null) {
  ...
  protected FrameAndSubId getFrameAndSubIdFromGenericKey(FieldKey genericKey) {
    ...
    ID3v24FieldKey id3v24FieldKey =
      ID3v24Frames.getInstanceOf().getId3KeyFromGenericKey(genericKey);
    if (id3v24FieldKey == null) {
