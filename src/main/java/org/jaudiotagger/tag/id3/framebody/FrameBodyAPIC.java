...
public class FrameBodyAPIC
  ...
  public FrameBodyAPIC(FrameBodyPIC body) {
    ...
    this.setObjectValue(
      ...
      ImageFormats.getMimeTypeForFormat(
        (String) body.getObjectValue(DataTypes.OBJ_IMAGE_FORMAT)
      )
    );
    this.setObjectValue(
  ...
  public String getUserFriendlyValue() {
    if (getImageData() != null) {
      return (
        getMimeType() + ":" + getDescription() + ":" + getImageData().length
      );
    } else {
  ...
  protected void setupObjectList() {
    ...
    objectList.add(
      new ByteArraySizeTerminated(DataTypes.OBJ_PICTURE_DATA, this)
    );
  }
  ...
  public String getImageUrl() {
    if (isImageUrl()) {
      return new String(
        ((byte[]) getObjectValue(DataTypes.OBJ_PICTURE_DATA)),
