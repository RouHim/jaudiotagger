...
public class FrameBodyTXXX
  ...
  public static final String MUSICBRAINZ_WORK_COMPOSITION =
    "MUSICBRAINZ_WORK_COMPOSITION";
  public static final String MUSICBRAINZ_WORK_COMPOSITION_ID =
  ...
  public void write(ByteArrayOutputStream tagBuffer) {
    ...
    setTextEncoding(
      ID3TextEncodingConversion.getTextEncoding(getHeader(), getTextEncoding())
    );

    //Ensure valid for description
    if (
      !((TextEncodedStringNullTerminated) getObject(
          DataTypes.OBJ_DESCRIPTION
        )).canBeEncoded()
    ) {
      this.setTextEncoding(
        ID3TextEncodingConversion.getUnicodeTextEncoding(getHeader())
      );
    }
  ...
  protected void setupObjectList() {
    ...
    objectList.add(
      new TextEncodedStringNullTerminated(DataTypes.OBJ_DESCRIPTION, this)
    );
    objectList.add(
      new TextEncodedStringSizeTerminated(DataTypes.OBJ_TEXT, this)
    );
  }
}
