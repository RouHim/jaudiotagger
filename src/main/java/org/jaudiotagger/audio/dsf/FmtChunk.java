...
public class FmtChunk {
    ...
    public GenericAudioHeader readChunkData(DsdChunk dsd, FileChannel fc)
            throws IOException {
        long sizeExcludingChunkHeader =
                chunkSizeLength - (IffHeaderChunk.SIGNATURE_LENGTH + CHUNKSIZE_LENGTH);
        ByteBuffer audioData = Utils.readFileDataIntoBufferLE(
                fc,
                (int) sizeExcludingChunkHeader
        );
    ...
    private GenericAudioHeader readAudioInfo(
            DsdChunk dsd,
            ByteBuffer audioInfoChunk
    ) {
        ...
        log.debug("Created audio header: " + audioHeader);
        return audioHeader;
    }
}
