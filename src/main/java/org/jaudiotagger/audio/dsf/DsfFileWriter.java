...
public class DsfFileWriter extends AudioFileWriter2 {
    ...
    protected void writeTag(Tag tag, FileChannel fc, final String fileName)
                ...
                if (dsd.getMetadataOffset() > 0) {
                    ...
                    if (id3Chunk != null) {
                        ...
                        throw new CannotWriteException(
                                fileName + "Could not find existing ID3v2 Tag"
                        );
                    }
                } else {
     ...
     */
    public ByteBuffer convert(final AbstractID3v2Tag tag)
            throws UnsupportedEncodingException {
        try {
     ...
     */
    @Override
    protected void deleteTag(Tag tag, FileChannel fc, final String fileName)
            throws CannotWriteException {
        try {
            DsdChunk dsd = DsdChunk.readChunk(
                    Utils.readFileDataIntoBufferLE(fc, DsdChunk.DSD_HEADER_LENGTH)
