...
public class DsfFileReader extends AudioFileReader2 {
    ...
    protected GenericAudioHeader getEncodingInfo(
        ...
        DsdChunk dsd = DsdChunk.readChunk(
                Utils.readFileDataIntoBufferLE(fc, DsdChunk.DSD_HEADER_LENGTH)
        );
        if (dsd != null) {
            ByteBuffer fmtChunkBuffer = Utils.readFileDataIntoBufferLE(
                    fc,
                    IffHeaderChunk.SIGNATURE_LENGTH + CHUNKSIZE_LENGTH
            );
            ...
            if (fmt != null) {
                ...
                throw new CannotReadException(
                        fileName +
                                " Not a valid dsf file. Content does not include 'fmt ' chunk"
                );
            }
        } else {
            throw new CannotReadException(
                    fileName + " Not a valid dsf file. Content does not start with 'DSD '"
            );
        }
    }

    @Override
    protected Tag getTag(FileChannel fc, final String fileName)
            throws CannotReadException, IOException {
        DsdChunk dsd = DsdChunk.readChunk(
                Utils.readFileDataIntoBufferLE(fc, DsdChunk.DSD_HEADER_LENGTH)
        );
     ...
     */
    private Tag readTag(FileChannel fc, DsdChunk dsd, String fileName)
            throws CannotReadException, IOException {
        if (dsd.getMetadataOffset() > 0) {
            fc.position(dsd.getMetadataOffset());
            ID3Chunk id3Chunk = ID3Chunk.readChunk(
                    Utils.readFileDataIntoBufferLE(fc, (int) (fc.size() - fc.position()))
            );
            if (id3Chunk != null) {
                int version = id3Chunk
                        .dataBuffer()
                        .get(AbstractID3v2Tag.FIELD_TAG_MAJOR_VERSION_POS);
                try {
                    switch (version) {
                        ...
                        default:
                            log.warn(
                                    fileName +
                                            " Unknown ID3v2 version " +
                                            version +
                                            ". Returning an empty ID3v2 Tag."
                            );
                            return null;
                    }
                } catch (TagException e) {
                    throw new CannotReadException(
                            fileName + " Could not read ID3v2 tag:corruption"
                    );
                }
            } else {
