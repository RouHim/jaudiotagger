...
public class WavTagWriter {
    ...
    public void delete(Tag tag, FileChannel fc) throws CannotWriteException {
        log.info(loggingName + " Deleting metadata from file");
        try {
            ...
            if (existingTag.isExistingId3Tag() && existingTag.isExistingInfoTag()) {
                ...
                if (fs.isContiguous) {
                    ...
                    //Slower
                        ...
                        if (fs.isInfoTagFirst) {
                            final int lengthTagChunk =
                                    (int) (existingTag.getEndLocationInFileOfId3Chunk() -
                                            existingTag.getInfoTag().getStartLocationInFile());
                            deleteTagChunk(
                                    fc,
                                    (int) existingTag.getEndLocationInFileOfId3Chunk(),
                            ...
                            final int lengthTagChunk = (int) (existingTag
                                    .getInfoTag()
                ...
                //Tricky to delete both because once one is deleted affects the location of the other
                    ...
                    ChunkHeader infoChunkHeader = seekToStartOfListInfoMetadata(
                            fc,
                            existingTag
                    );

                    ChunkHeader id3ChunkHeader = seekToStartOfId3Metadata(
                            fc,
                            existingTag
                    );
                    ...
                    if (isInfoTagAtEndOfFileAllowingForPaddingByte(existingTag, fc)) {
                        ...
                        deleteId3TagChunk(fc, existingTag, id3ChunkHeader);
                            ...
                            isID3TagAtEndOfFileAllowingForPaddingByte(existingTag, fc)
                    ) {
                        fc.truncate(existingTag.getStartLocationInFileOfId3Chunk());
            ...
            //Delete Info if exists
                ...
                WavInfoTag existingInfoTag = existingTag.getInfoTag();
                ChunkHeader chunkHeader = seekToStartOfListInfoMetadata(
                        fc,
                        existingTag
                );
                ...
                if (existingInfoTag.getEndLocationInFile() == fc.size()) {
                    log.info(
                            loggingName +
                                    ":Setting new length to:" +
                                    existingInfoTag.getStartLocationInFile()
                    );
                    fc.truncate(existingInfoTag.getStartLocationInFile());
                } else {
     ...
     * @throws CannotWriteException
     */
    private WavTag getExistingMetadata(FileChannel fc)
     ...
     * @throws CannotWriteException
     */
    private ChunkHeader seekToStartOfListInfoMetadata(
            FileChannel fc,
            WavTag existingTag
    ) throws IOException, CannotWriteException {
        ...
        if (!WavChunkType.LIST.getCode().equals(chunkHeader.getID())) {
            throw new CannotWriteException(
                    loggingName +
                            " Unable to find List chunk at original location has file been modified externally"
            );
        }
        return chunkHeader;
    }
     ...
     * @throws CannotWriteException
     */
    private ChunkHeader seekToStartOfId3Metadata(
            FileChannel fc,
            WavTag existingTag
    ) throws IOException, CannotWriteException {
        ...
        if (!WavChunkType.ID3.getCode().equals(chunkHeader.getID())) {
            throw new CannotWriteException(
                    loggingName +
                            " Unable to find ID3 chunk at original location has file been modified externally"
            );
        }
        return chunkHeader;
    }
     ...
     * @throws IOException
     */
    private void deleteInfoTagChunk(
        ...
        final int lengthTagChunk =
                (int) chunkHeader.getSize() + ChunkHeader.CHUNK_HEADER_SIZE;
        deleteTagChunk(
                fc,
                existingInfoTag.getEndLocationInFile().intValue(),
                lengthTagChunk
        );
    }
     ...
     * @throws IOException
     */
    private void deleteId3TagChunk(
            FileChannel fc,
            final WavTag existingTag,
            final ChunkHeader chunkHeader
    ) throws IOException {
     ...
     * @throws IOException
     */
    private void deleteTagChunk(
            final FileChannel fc,
            int endOfExistingChunk,
            final int lengthTagChunk
    ) throws IOException {
        ...
        final ByteBuffer buffer = ByteBuffer.allocate(
                (int) TagOptionSingleton.getInstance().getWriteChunkSize()
        );
        ...
        log.debug(loggingName + " Setting new length to:" + newLength);
     ...
     * @throws IOException
     */
    private BothTagsFileStructure checkExistingLocations(
            WavTag wavTag,
            FileChannel fc
    ) throws IOException {
        BothTagsFileStructure fs = new BothTagsFileStructure();
        if (
                wavTag.getInfoTag().getStartLocationInFile() <
                        wavTag.getID3Tag().getStartLocationInFile()
        ) {
            fs.isInfoTagFirst = true;
            //Must allow for odd size chunks
            if (
                    Math.abs(
                            wavTag.getInfoTag().getEndLocationInFile() -
            ...
            if (
                    Math.abs(
                            wavTag.getID3Tag().getEndLocationInFile() -
                                    wavTag.getInfoTag().getStartLocationInFile()
                    ) <=
                            1
            ) {
     ...
     * @throws IOException
     */
    private boolean isID3TagAtEndOfFileAllowingForPaddingByte(
            WavTag existingTag,
            FileChannel fc
    ) throws IOException {
        return (
     ...
     * @throws IOException
     */
    private boolean isInfoTagAtEndOfFileAllowingForPaddingByte(
            WavTag existingTag,
            FileChannel fc
    ) throws IOException {
        return (
                (existingTag.getInfoTag().getEndLocationInFile() == fc.size()) ||
    ...
    public void write(final Tag tag, FileChannel fc) throws CannotWriteException {
        log.debug(loggingName + " Writing tag to file:start");
        ...
        try {
            ...
            //Invalid Option, should never happen
                ...
                throw new RuntimeException(
                        loggingName + " No setting for:WavSaveOptions"
                );
            }
            rewriteRiffHeaderSize(fc);
        } catch (IOException ioe) {
     ...
     */
    private void writeInfoDataToFile(
            FileChannel fc,
            final ByteBuffer bb,
        ...
        //Write LIST header
        final ByteBuffer listHeaderBuffer = ByteBuffer.allocate(
                ChunkHeader.CHUNK_HEADER_SIZE
        );
        listHeaderBuffer.order(ByteOrder.LITTLE_ENDIAN);
        listHeaderBuffer.put(
                WavChunkType.LIST.getCode().getBytes(StandardCharsets.US_ASCII)
        );
     ...
     * @throws IOException
     */
    private void writeInfoDataToFile(final FileChannel fc, final ByteBuffer bb)
            throws IOException {
     ...
     * @throws java.io.IOException
     */
    private void writeID3DataToFile(final FileChannel fc, final ByteBuffer bb)
            throws IOException {
        if (Utils.isOddLength(fc.position())) {
            writePaddingToFile(fc, 1);
        }
        ...
        final ByteBuffer listBuffer = ByteBuffer.allocate(
                ChunkHeader.CHUNK_HEADER_SIZE
        );
        listBuffer.order(ByteOrder.LITTLE_ENDIAN);
        listBuffer.put(
                WavChunkType.ID3.getCode().getBytes(StandardCharsets.US_ASCII)
        );
     ...
     * @throws IOException
     */
    private void writePaddingToFile(final FileChannel fc, final int paddingSize)
            throws IOException {
    ...
    public ByteBuffer convertInfoChunk(final WavTag tag) {
        try {
            ...
            for (TagField nextField : fields) {
                ...
                WavInfoIdentifier wii = WavInfoIdentifier.getByByFieldKey(
                        FieldKey.valueOf(next.getId())
                );
                baos.write(wii.getCode().getBytes(StandardCharsets.US_ASCII));
                log.debug(
                        loggingName + " Writing:" + wii.getCode() + ":" + next.getContent()
                );

                //TODO Is UTF8 allowed format
                byte[] contentConvertedToBytes = next
                        .getContent()
                ...
                if (wii == WavInfoIdentifier.TRACKNO) {
                    if (TagOptionSingleton.getInstance().isWriteWavForTwonky()) {
                        baos.write(
                                WavInfoIdentifier.TWONKY_TRACKNO.getCode().getBytes(
                        ...
                        log.debug(
                                loggingName +
                                        " Writing:" +
                                        WavInfoIdentifier.TWONKY_TRACKNO.getCode() +
                                        ":" +
                                        next.getContent()
                        );
            ...
            while (ti.hasNext()) {
                ...
                log.debug(
                        loggingName + " Writing:" + next.getId() + ":" + next.getContent()
                );
                byte[] contentConvertedToBytes = next
                        .getContent()
                        .getBytes(StandardCharsets.UTF_8);
            ...
            infoHeaderBuffer.put(
                    WavChunkType.INFO.getCode().getBytes(StandardCharsets.US_ASCII)
            );
            infoHeaderBuffer.flip();

            //Construct a single ByteBuffer from both
            ByteBuffer listInfoBuffer = ByteBuffer.allocateDirect(
                    infoHeaderBuffer.limit() + infoBuffer.limit()
            );
     ...
     * @throws IOException
     */
    private void writeInfoChunk(
            FileChannel fc,
            final WavInfoTag existingInfoTag,
            ByteBuffer newTagBuffer
    ) throws IOException {
        ...
        if (existingInfoTag.getSizeOfTag() >= newInfoTagSize) {
            ...
            if (existingInfoTag.getSizeOfTag() > newInfoTagSize) {
                writePaddingToFile(
                        fc,
                        (int) (existingInfoTag.getSizeOfTag() - newInfoTagSize)
                );
            }
        }
    ...
    private void saveBoth(WavTag wavTag, FileChannel fc, final WavTag existingTag)
        ...
        if (existingTag.isExistingInfoTag() && existingTag.isExistingId3Tag()) {
            ...
            //tags and start again
                    ...
                    WavChunkSummary.isOnlyMetadataTagsAfterStartingMetadataTag(existingTag)
            ) {
                ...
                throw new CannotWriteException(
                        loggingName +
                                " Metadata tags are corrupted and not at end of file so cannot be fixed"
                );
            }
        }
        //If only INFO chunk exists
        else if (
                existingTag.isExistingInfoTag() && !existingTag.isExistingId3Tag()
        ) {
            if (!existingTag.isIncorrectlyAlignedTag()) {
                ChunkHeader infoChunkHeader = seekToStartOfListInfoMetadata(
                        fc,
        ...
        //If only ID3 chunk exists
                ...
                existingTag.isExistingId3Tag() && !existingTag.isExistingInfoTag()
        ) {
            if (!existingTag.isIncorrectlyAlignedTag()) {
     ...
     * @throws IOException
     */
    private void writeBothTags(
            FileChannel fc,
            ByteBuffer infoTagBuffer,
            ByteBuffer id3TagBuffer
    ) throws IOException {
        if (
                TagOptionSingleton.getInstance().getWavSaveOrder() ==
     ...
     * @throws IOException
     */
    private void saveActiveExisting(
            WavTag wavTag,
            FileChannel fc,
            final WavTag existingTag
    ) throws CannotWriteException, IOException {
     ...
     * @throws IOException
     */
    private void deleteExistingMetadataTagsToEndOfFile(
            final FileChannel fc,
            final WavTag existingTag
    ) throws IOException {
        ChunkSummary precedingChunk =
                WavChunkSummary.getChunkBeforeFirstMetadataTag(existingTag);
        //Preceding chunk ends on odd boundary
        if (!Utils.isOddLength(precedingChunk.getEndLocation())) {
            log.error(
                    loggingName +
                            " Truncating corrupted metadata tags from:" +
    ...
    class InfoFieldWriterOrderComparator implements Comparator<TagField> {
        ...
        public int compare(TagField field1, TagField field2) {
            WavInfoIdentifier code1 = WavInfoIdentifier.getByByFieldKey(
                    FieldKey.valueOf(field1.getId())
            );
            WavInfoIdentifier code2 = WavInfoIdentifier.getByByFieldKey(
                    FieldKey.valueOf(field2.getId())
            );
