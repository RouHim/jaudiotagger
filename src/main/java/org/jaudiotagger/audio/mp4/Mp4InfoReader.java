...
public class Mp4InfoReader {
    ...
    public GenericAudioHeader read(RandomAccessFile raf)
        ...
        //File Identification
        if (mp4 == null || mp4.ftyp() == null) {
            throw new CannotReadException(
                    ErrorMessage.MP4_FILE_NOT_CONTAINER.getMsg()
            );
        }
        info.setBrand(mp4.ftyp().getMajorBrand());

        //Get to the facts everything we are interested in is within the moov box, so just load data from file
        //once so no more file I/O needed
        MovieBox moov = mp4.moov();
        if (moov == null) {
        ...
        SoundMediaHeaderBox smhd = NodeBox.findFirstPath(
                trak.getMdia().getMinf(),
                SoundMediaHeaderBox.class,
                Box.path("smhd")
        );
        if (smhd == null) {
            VideoMediaHeaderBox vmhd = NodeBox.findFirstPath(
                    trak.getMdia().getMinf(),
                    VideoMediaHeaderBox.class,
                    Box.path("vmhd")
            );
            //try easy check to confirm that it is video
            if (vmhd != null) {
                throw new CannotReadVideoException(
                        ErrorMessage.MP4_FILE_IS_VIDEO.getMsg()
                );
            } else {
                throw new CannotReadException(ErrorMessage.MP4_FILE_NOT_AUDIO.getMsg());
            }
        ...
        if (stsd != null) {
            ...
            EsdsBox esds = NodeBox.findFirstPath(
                    stsd,
                    EsdsBox.class,
                    Box.path("mp4a.esds")
            );
            EsdsBox esdsProtected = NodeBox.findFirstPath(
                    stsd,
                    EsdsBox.class,
                    Box.path("drms.esds")
            );
            AudioSampleEntry alac = NodeBox.findFirstPath(
                    stsd,
            ...
            if (esds != null) {
                ...
                info.setBitRate(
                        (int) (alac.getSampleRate() / alac.getSampleSize() / 8)
                );
