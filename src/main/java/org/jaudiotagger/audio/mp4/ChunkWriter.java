...
public class ChunkWriter {
  ...
  public ChunkWriter(
    ...
    int size;
    if (stco != null) size = stco.getChunkOffsets().length;
    else size = co64.getChunkOffsets().length;
    this.input = input;
  ...
  public void apply() {
    NodeBox stbl = NodeBox.findFirstPath(
      trak,
      NodeBox.class,
      Box.path("mdia.minf.stbl")
    );
    ...
    SampleSizesBox stsz = sampleCount != 0
      ? SampleSizesBox.createSampleSizesBox(sampleSize, sampleCount)
  ...
  private SeekableByteChannel getInput(Chunk chunk) {
    ...
    if (se.getDrefInd() != 1) throw new RuntimeException(
      "Multiple sample entries not supported"
    );
    return input;
  }
  ...
  public void write(Chunk chunk) throws IOException {
    ...
    if (chunk.getSampleSize() == Chunk.UNEQUAL_SIZES) {
      if (sampleCount != 0) throw new RuntimeException(
        "Mixed chunks unsupported 1."
      );
      sampleSizes.addAll(chunk.getSampleSizes());
    } else {
      if (sampleSizes.size() != 0) throw new RuntimeException(
        "Mixed chunks unsupported 2."
      );
      if (sampleCount == 0) {
