...
public class ChunkReader {
  ...
  public ChunkReader(TrakBox trakBox, SeekableByteChannel inputs) {
    ...
    if (stco != null) chunkOffsets = stco.getChunkOffsets();
    else chunkOffsets = co64.getChunkOffsets();
  ...
  public Chunk next() throws IOException {
    if (curChunk >= chunkOffsets.length) return null;

    if (
      s2cIndex + 1 < sampleToChunk.length &&
      curChunk + 1 == sampleToChunk[s2cIndex + 1].getFirst()
    ) s2cIndex++;
    ...
    if (ttsSubInd + sampleCount <= tts[ttsInd].getSampleCount()) {
      ...
      for (int i = 0; i < sampleCount; i++) {
        if (
          ttsSubInd >= tts[ttsInd].getSampleCount() && ttsInd < tts.length - 1
        ) {
          ttsSubInd = 0;
          ++ttsInd;
        }
