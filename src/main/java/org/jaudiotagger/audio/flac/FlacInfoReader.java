...
public class FlacInfoReader {
    ...
    public FlacAudioHeader read(FileChannel fc, final String fileName)
            throws CannotReadException, IOException {
        log.debug(fileName + ":start");
        ...
        if (mbdsi == null) {
            throw new CannotReadException(
                    fileName + ":Unable to find Flac StreamInfo"
            );
        }
        ...
        info.setBitRate(
                computeBitrate(info.getAudioDataLength(), mbdsi.getPreciseLength())
        );
        return info;
    }
    ...
    public int countMetaBlocks(File f) throws CannotReadException, IOException {
        try (FileChannel fc = new RandomAccessFile(f, "r").getChannel()) {
            ...
            while (!isLastBlock) {
                MetadataBlockHeader mbh = MetadataBlockHeader.readHeader(fc);
                log.debug(f + ":Found block:" + mbh.getBlockType());
                fc.position(fc.position() + mbh.getDataLength());
                isLastBlock = mbh.isLastBlock();
                count++;
            }
