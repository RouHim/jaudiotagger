...
public class FlacTagWriter {
     ...
     */
    public void delete(Tag tag, FileChannel channel, final String fileName)
            throws CannotWriteException {
        //This will save the file without any Comment or PictureData blocks
        FlacTag emptyTag = new FlacTag(
                null,
                new ArrayList<MetadataBlockDataPicture>()
        );
     ...
     * @throws CannotWriteException
     */
    public void write(Tag tag, FileChannel fc, final String fileName)
            throws CannotWriteException {
        log.debug(fileName + " Writing tag");
        try {
                ...
                try {
                    ...
                    if (mbh.getBlockType() != null) {
                        switch (mbh.getBlockType()) {
                            case STREAMINFO: {
                                blockInfo.streamInfoBlock = new MetadataBlock(
                                        mbh,
                                        new MetadataBlockDataStreamInfo(mbh, fc)
                                );
                                break;
                            }
                            case VORBIS_COMMENT:
                            ...
                            case APPLICATION: {
                                MetadataBlockData mbd = new MetadataBlockDataApplication(
                                        mbh,
                                        fc
                                );
                                blockInfo.metadataBlockApplication.add(
                                        new MetadataBlock(mbh, mbd)
                                );
                                break;
                            }
                            case SEEKTABLE: {
                                MetadataBlockData mbd = new MetadataBlockDataSeekTable(mbh, fc);
                                blockInfo.metadataBlockSeekTable.add(
                                        new MetadataBlock(mbh, mbd)
                                );
                                break;
                            }
                            case CUESHEET: {
                                ...
                                blockInfo.metadataBlockCueSheet.add(
                                        new MetadataBlock(mbh, mbd)
                                );
                                break;
                            }
            ...
            if (
                    (availableRoom == neededRoom) ||
                            (availableRoom > neededRoom + MetadataBlockHeader.HEADER_LENGTH)
            ) {
                log.debug(fileName + " Room to Rewrite");
            ...
            //Need to move audio
                ...
                log.debug(
                        fileName +
                                ":Audio must be shifted " +
                                "NewTagSize:" +
                                newTagSize +
                                ":AvailableRoom:" +
                                availableRoom +
                                ":MinimumAdditionalRoomRequired:" +
     ...
     */
    private void insertUsingChunks(
            String file,
            Tag tag,
            FileChannel fc,
        ...
        log.debug(file + " Audio needs shifting:" + extraSpaceRequired);
     ...
     * @throws IOException
     */
    private void writeOtherMetadataBlocks(
            FileChannel fc,
            MetadataBlockInfo blockInfo
    ) throws IOException {
        ...
        fc.write(
                ByteBuffer.wrap(
                        blockInfo.streamInfoBlock.getHeader().getBytesWithoutIsLastBlockFlag()
                )
        );
        ...
        for (MetadataBlock aMetadataBlockApplication : blockInfo.metadataBlockApplication) {
            fc.write(
                    ByteBuffer.wrap(
                            aMetadataBlockApplication.getHeader().getBytesWithoutIsLastBlockFlag()
                    )
            );
        ...
        for (MetadataBlock aMetadataBlockSeekTable : blockInfo.metadataBlockSeekTable) {
            fc.write(
                    ByteBuffer.wrap(
                            aMetadataBlockSeekTable.getHeader().getBytesWithoutIsLastBlockFlag()
                    )
            );
        ...
        for (MetadataBlock aMetadataBlockCueSheet : blockInfo.metadataBlockCueSheet) {
            fc.write(
                    ByteBuffer.wrap(
                            aMetadataBlockCueSheet.getHeader().getBytesWithoutIsLastBlockFlag()
                    )
            );
     ...
     * @throws UnsupportedEncodingException
     */
    private void insertUsingDirectBuffer(
            String fileName,
            Tag tag,
            FileChannel fc,
            MetadataBlockInfo blockInfo,
        ...
        ByteBuffer audioData = ByteBuffer.allocateDirect(
                (int) (fc.size() - fc.position())
        );
        fc.read(audioData);
        audioData.flip();
     ...
     * @throws UnsupportedEncodingException
     */
    private void insertTagAndShift(
            String fileName,
            Tag tag,
            FileChannel fc,
            MetadataBlockInfo blockInfo,
        ...
        long targetSizeBeforeAudioData =
                headerLength + neededRoom + FlacTagCreator.DEFAULT_PADDING;
        ...
        try {
            ...
            insertTagAndShiftViaMappedByteBuffer(
                    tag,
                    mappedFile,
                    fc,
     ...
     * @throws UnsupportedEncodingException
     */
    private void insertTagAndShiftViaMappedByteBuffer(
            Tag tag,
        ...
        int currentEndOfTagsPosition = safeLongToInt(
                (targetSizeBeforeAudioData - FlacTagCreator.DEFAULT_PADDING) -
                        neededRoom +
                        availableRoom
        );
        int lengthDiff = safeLongToInt(totalTargetSize - currentEndOfFilePosition);
        final int BLOCK_SIZE = safeLongToInt(
                TagOptionSingleton.getInstance().getWriteChunkSize()
        );
    ...
    private static class MetadataBlockInfo {
        ...
        private final List<MetadataBlock> metadataBlockPadding = new ArrayList<
                MetadataBlock
                >(1);
        private final List<MetadataBlock> metadataBlockApplication = new ArrayList<
                MetadataBlock
                >(1);
        private final List<MetadataBlock> metadataBlockSeekTable = new ArrayList<
                MetadataBlock
                >(1);
        private final List<MetadataBlock> metadataBlockCueSheet = new ArrayList<
                MetadataBlock
                >(1);
