...
public class AiffTagWriter {
   ...
   */
  private AiffTag getExistingMetadata(
    FileChannel channel,
   ...
   * @throws CannotWriteException
   */
  private ChunkHeader seekToStartOfMetadata(
    FileChannel fc,
    AiffTag existingTag,
    String fileName
  ) throws IOException, CannotWriteException {
    ...
    if (!AiffChunkType.TAG.getCode().equals(chunkHeader.getID())) {
      throw new CannotWriteException(
        fileName +
          " Unable to find ID3 chunk at expected location:" +
   ...
   * @throws IOException
   */
  private boolean isAtEndOfFileAllowingForPaddingByte(
    AiffTag existingTag,
    FileChannel fc
  ) throws IOException {
    return (
   ...
   */
  public void delete(final Tag tag, FileChannel fc, final String fileName)
    throws CannotWriteException {
    try {
      logger.error(fileName + " Deleting tag from file");
      final AiffTag existingTag = getExistingMetadata(fc, fileName);
      fc.position(0);
      if (
        existingTag.isExistingId3Tag() &&
        existingTag.getID3Tag().getStartLocationInFile() != null
      ) {
        ChunkHeader chunkHeader = seekToStartOfMetadata(
          fc,
          existingTag,
          fileName
        );
        if (isAtEndOfFileAllowingForPaddingByte(existingTag, fc)) {
          ...
          logger.error(fileName + " Deleting tag chunk");
          deleteTagChunk(fc, existingTag, chunkHeader, fileName);
        }
        rewriteRiffHeaderSize(fc);
      }
      logger.error(fileName + " Deleted tag from fc");
  ...
  private void deleteTagChunk(
    ...
    int lengthTagChunk =
      (int) tagChunkHeader.getSize() + ChunkHeader.CHUNK_HEADER_SIZE;
    if (Utils.isOddLength(lengthTagChunk)) {
      if (
        existingTag.getStartLocationInFileOfId3Chunk() + lengthTagChunk <
        fc.size()
      ) {
        lengthTagChunk++;
      }
    ...
    logger.error(
      fileName +
        " Size of id3 chunk to delete is:" +
        lengthTagChunk +
        ":Location:" +
        existingTag.getStartLocationInFileOfId3Chunk()
    );
    ...
    logger.error(fileName + " Setting new length to:" + newLength);
   ...
   * @throws IOException
   */
  private void deleteRemainderOfFile(
    FileChannel fc,
    final AiffTag existingTag,
    String fileName
  ) throws IOException {
    ChunkSummary precedingChunk =
      AiffChunkSummary.getChunkBeforeStartingMetadataTag(existingTag);
    if (!Utils.isOddLength(precedingChunk.getEndLocation())) {
      logger.error(
        fileName +
          " Truncating corrupted ID3 tags from:" +
          (existingTag.getStartLocationInFileOfId3Chunk() - 1)
      );
  ...
  private void deleteTagChunkUsingChannelTransfer(
    ...
    for (
      ...
      (read = channel.transferFrom(channel, position, newLength - position)) <
      newLength - position;
      position += read
  ...
  private void deleteTagChunkUsingSmallByteBufferSegments(
    final AiffTag existingTag,
    final FileChannel channel,
    final long newLength,
   ...
   * @throws CannotWriteException
   */
  public void write(final Tag tag, FileChannel fc, final String fileName)
    throws CannotWriteException {
    logger.error(fileName + " Writing Aiff tag to fc");
    ...
    try {
      ...
      if (
        ...
        if (!existingTag.isIncorrectlyAlignedTag()) {
          final ChunkHeader chunkHeader = seekToStartOfMetadata(
            fc,
            existingTag,
            fileName
          );
        ...
        //ID3 tags as we only want one ID3 tag.
          ...
          throw new CannotWriteException(
            fileName +
              " Metadata tags are corrupted and not at end of fc so cannot be fixed"
          );
        }
      }
      //New Tag
   ...
   */
  public ByteBuffer convert(final AiffTag tag, AiffTag existingTag)
    throws UnsupportedEncodingException {
    try {
