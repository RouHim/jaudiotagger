...
public class AiffInfoReader extends AiffChunkReader {
    ...
    protected GenericAudioHeader read(FileChannel fc, final String fileName)
            throws CannotReadException, IOException {
        log.debug(
                fileName + " Reading AIFF file size:" + Hex.asDecAndHex(fc.size())
        );
        ...
        while (fc.position() < fc.size()) {
            if (!readChunk(fc, aiffAudioHeader, fileName)) {
                log.error(fileName + " UnableToReadProcessChunk");
                break;
            }
        }
    ...
    private void calculateBitRate(GenericAudioHeader info) {
        if (info.getAudioDataLength() != null) {
            info.setBitRate(
                    (int) (Math.round(
                            (info.getAudioDataLength() * Utils.BITS_IN_BYTE_MULTIPLIER) /
    ...
    private boolean readChunk(
        ...
        log.debug(fileName + " Reading Info Chunk");
        final Chunk chunk;
        ...
        if (chunk != null) {
            if (!chunk.readChunk()) {
                log.error(fileName + "ChunkReadFail:" + chunkHeader.getID());
                return false;
            }
        } else {
        ...
        if (chunkType != null) {
            switch (chunkType) {
                ...
                case SOUND:
                    ...
                    aiffAudioHeader.setAudioDataEndPosition(
                            fc.position() + chunkHeader.getSize()
                    );
