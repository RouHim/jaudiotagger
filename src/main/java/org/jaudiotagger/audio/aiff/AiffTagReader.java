...
public class AiffTagReader extends AiffChunkReader {
     ...
     * @throws IOException
     */
    public AiffTag read(FileChannel channel, final String fileName)
     ...
     */
    private boolean readChunk(FileChannel fc, AiffTag aiffTag, String fileName)
            throws IOException {
        log.debug(fileName + " Reading Tag Chunk");
        ...
        if (chunkType != null && chunkType == AiffChunkType.TAG) {
            ...
            //else otherwise we discard because the first one found is the one that will be used by other apps
            {
                log.warn(
                        fileName +
                                " Ignoring ID3Tag because already have one:" +
                                chunkHeader.getID() +
                                ":" +
                                chunkHeader.getStartLocationInFile() +
        ...
        //didn't write padding byte
            ...
            log.warn(
                    fileName +
                            "Found Corrupt ID3 Chunk, starting at Odd Location:" +
                            chunkHeader.getID() +
                            ":" +
                            Hex.asDecAndHex(chunkHeader.getStartLocationInFile() - 1) +
                            ":sizeIncHeader:" +
        ...
        //Other Special handling for ID3Tags
                ...
                chunkType != null && chunkType == AiffChunkType.CORRUPT_TAG_EARLY
        ) {
            log.warn(
                    fileName +
                            " Found Corrupt ID3 Chunk, starting at Odd Location:" +
