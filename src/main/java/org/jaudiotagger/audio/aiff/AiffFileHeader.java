...
public class AiffFileHeader {
  ...
  private static final Logger logger = LoggerFactory.getLogger(
    "org.jaudiotagger.audio.aiff.AudioFileHeader"
  );
  ...
  public long readHeader(
    ...
    if (bytesRead < HEADER_LENGTH) {
      throw new IOException(
        fileName +
          " AIFF:Unable to read required number of databytes read:" +
          bytesRead +
          ":required:" +
          HEADER_LENGTH
      );
    ...
    if (FORM.equals(signature)) {
      ...
      logger.error(
        fileName + " Reading AIFF header size:" + Hex.asDecAndHex(chunkSize)
      );
      ...
      throw new CannotReadException(
        fileName + "Not an AIFF file: incorrect signature " + signature
      );
    }
  }
  ...
  private void readFileType(
    ...
    if (AIFF.getCode().equals(type)) {
      ...
      throw new CannotReadException(
        "Invalid AIFF file: Incorrect file type info " + type
      );
    }
  }
}
