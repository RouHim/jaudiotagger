...
public abstract class AudioFileReader2 extends AudioFileReader {
  ...
  public AudioFile read(File f) throws CannotReadException, IOException {
    ...
    if (f.length() <= MINIMUM_SIZE_FOR_VALID_AUDIO_FILE) {
      throw new CannotReadException(
        ErrorMessage.GENERAL_READ_FAILED_FILE_TOO_SMALL.getMsg(f)
      );
    }
   ...
   * @throws IOException
   */
  protected abstract GenericAudioHeader getEncodingInfo(
    FileChannel channel,
   ...
   * @throws IOException
   */
  protected abstract Tag getTag(FileChannel channel, final String fileName)
    throws CannotReadException, IOException;
