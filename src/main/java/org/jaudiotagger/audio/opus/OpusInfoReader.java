...
public class OpusInfoReader {
  ...
  private static final Logger logger = LoggerFactory.getLogger(
    "org.jaudiotagger.audio.opus.atom"
  );
  ...
  public GenericAudioHeader read(RandomAccessFile raf)
    ...
    raf.read(vorbisData);
    OpusVorbisIdentificationHeader opusIdHeader =
      new OpusVorbisIdentificationHeader(vorbisData);
    ...
    if (last == null) {
      throw new CannotReadException(
        "Opus file contains ID and Comment headers but no audio content"
      );
    }

    info.setNoOfSamples(
      last.getAbsoluteGranulePosition() - opusIdHeader.getPreSkip()
    );
  ...
  private OggPageHeader lastValidHeader(RandomAccessFile raf)
    throws IOException {
