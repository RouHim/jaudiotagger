...
public class OpusVorbisTagWriter {
    ...
    private final OggVorbisCommentTagCreator tc = new OggVorbisCommentTagCreator(
            new byte[0],
            OpusHeader.TAGS_CAPTURE_PATTERN_AS_BYTES,
            false
    );
    ...
    public void write(Tag tag, RandomAccessFile raf, RandomAccessFile rafTemp)
        ...
        log.debug("Starting to write file: " + raf);
        ...
        while (originalHeaders.get(0).header().isContinuedPage()) {
            originalHeaders.remove(0); // skip continued tag pages
        }

        final int fullPagesNeeded =
                newComment.capacity() / OggPageHeader.MAXIMUM_PAGE_DATA_SIZE;
        final int pagesRemainder =
                newComment.capacity() % OggPageHeader.MAXIMUM_PAGE_DATA_SIZE;
        final int streamNo = identPage.header().getSerialNumber();
        int sequenceNo = 1;
        for (int page = 0; page < fullPagesNeeded; page++) {
            OggPageHeader header = OggPageHeader.createCommentHeader(
    ...
    private List<OggPage> readPages(FileChannel fi)
