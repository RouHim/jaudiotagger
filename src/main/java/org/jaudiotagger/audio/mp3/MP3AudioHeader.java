...
public class MP3AudioHeader implements AudioHeader {
  ...
  private static final SimpleDateFormat timeInFormat = new SimpleDateFormat(
    "ss",
    Locale.UK
  );
  private static final SimpleDateFormat timeOutFormat = new SimpleDateFormat(
    "mm:ss",
    Locale.UK
  );
  private static final SimpleDateFormat timeOutOverAnHourFormat =
    new SimpleDateFormat("kk:mm:ss", Locale.UK);
   ...
   */
  public MP3AudioHeader(final File seekFile)
    throws IOException, InvalidAudioFrameException {
    if (!seek(seekFile, 0)) {
      throw new InvalidAudioFrameException(
        "No audio header found within" + seekFile.getName()
      );
    }
  }
   ...
   * @throws InvalidAudioFrameException
   */
  public MP3AudioHeader(final File seekFile, long startByte)
    throws IOException, InvalidAudioFrameException {
    if (!seek(seekFile, startByte)) {
      throw new InvalidAudioFrameException(
        ErrorMessage.NO_AUDIO_HEADER_FOUND.getMsg(seekFile.getName())
      );
    }
  }
  ...
  public boolean seek(final File seekFile, long startByte) throws IOException {
    ...
    try {
      do {
        ...
        //logger.debug("fc:"+fc.position() + "bb"+bb.position());
        if (MPEGFrameHeader.isMPEGFrame(bb)) {
          try {
            ...
            if ((header = XingFrame.isXingFrame(bb, mp3FrameHeader)) != null) {
              ...
              (header = VbriFrame.isVbriFrame(bb, mp3FrameHeader)) != null
            ) {
              if (logger.isDebugEnabled()) {
   ...
   */
  private boolean isNextFrameValid(
    File seekFile,
    long filePointerCount,
    ByteBuffer bb,
    FileChannel fc
    ...
    if (
      bb.remaining() <=
      MIN_BUFFER_REMAINING_REQUIRED + mp3FrameHeader.getFrameLength()
    ) {
      logger.debug(
  ...
  protected void setNumberOfFrames() {
    numberOfFramesEstimate =
      (fileSize - startByte) / mp3FrameHeader.getFrameLength();
  ...
  protected void setTimePerFrame() {
    timePerFrame =
      mp3FrameHeader.getNoOfSamples() /
      mp3FrameHeader.getSamplingRate().doubleValue();
  ...
  protected void setBitRate() {
    if (mp3XingFrame != null && mp3XingFrame.isVbr()) {
      if (
        mp3XingFrame.isAudioSizeEnabled() && mp3XingFrame.getAudioSize() > 0
      ) {
        bitrate = (long) (((long) mp3XingFrame.getAudioSize() *
      ...
      if (mp3VbriFrame.getAudioSize() > 0) {
        bitrate = (long) (((long) mp3VbriFrame.getAudioSize() *
            CONVERTS_BYTE_TO_BITS) /
          (timePerFrame * getNumberOfFrames() * CONVERT_TO_KILOBITS));
      } else {
        bitrate = (long) (((fileSize - startByte) * CONVERTS_BYTE_TO_BITS) /
  ...
  public String getFormat() {
    return (
      mp3FrameHeader.getVersionAsString() +
      " " +
      mp3FrameHeader.getLayerAsString()
    );
  }
