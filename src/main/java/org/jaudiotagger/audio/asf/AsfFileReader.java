...
public class AsfFileReader extends AudioFileReader {
  ...
  private boolean determineVariableBitrate(final AsfHeader header) {
    ...
    if (extDesc != null) {
      final List<MetadataDescriptor> descriptors = extDesc.getDescriptorsByName(
        "IsVBR"
      );
  ...
  private GenericAudioHeader getAudioHeader(final AsfHeader header)
    ...
    info.setEncodingType(
      "ASF (audio): " + header.getAudioStreamChunk().getCodecDescription()
    );
    info.setLossless(
      header.getAudioStreamChunk().getCompressionFormat() ==
        AudioStreamChunk.WMA_LOSSLESS
    );
  ...
  protected GenericAudioHeader getEncodingInfo(final RandomAccessFile raf)
    ...
    try {
      ...
      if (e instanceof IOException) {
        ...
        throw new CannotReadException(
          "Failed to read. Cause: " + e.getMessage(),
          e
        );
      }
    }
  ...
  public AudioFile read(final File f)
    ...
    try {
      stream = new FullRequestInputStream(
        new BufferedInputStream(new FileInputStream(f))
      );
      final AsfHeader header = HEADER_READER.read(
      ...
      if (header == null) {
        throw new CannotReadException(
          ErrorMessage.ASF_HEADER_MISSING.getMsg(f.getAbsolutePath())
        );
      }
      if (header.getFileHeader() == null) {
        throw new CannotReadException(
          ErrorMessage.ASF_FILE_HEADER_MISSING.getMsg(f.getAbsolutePath())
        );
      }
      ...
      if (header.getFileHeader().getFileSize().longValue() != f.length()) {
        logger.warn(
          ErrorMessage.ASF_FILE_HEADER_SIZE_DOES_NOT_MATCH_FILE_SIZE.getMsg(
            f.getAbsolutePath(),
            header.getFileHeader().getFileSize().longValue(),
            f.length()
          )
        );
      }
