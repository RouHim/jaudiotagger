...
public final class TagConverter {
   ...
   */
  public static void assignCommonTagValues(
    Tag tag,
    ...
    if (!Utils.isBlank(tag.getFirst(FieldKey.ALBUM))) {
      tmp = new MetadataDescriptor(
        description.getContainerType(),
  ...
  public static AsfTag createTagOf(AsfHeader source) {
    ...
    for (int i = 0; i < ContainerType.values().length; i++) {
      MetadataContainer current = source.findMetadataContainer(
      ...
      if (current != null) {
        ...
        for (MetadataDescriptor descriptor : descriptors) {
          ...
          if (descriptor.getType() == MetadataDescriptor.TYPE_BINARY) {
            if (
              descriptor.getName().equals(AsfFieldKey.COVER_ART.getFieldName())
            ) {
              toAdd = new AsfTagCoverField(descriptor);
            } else if (
              descriptor
                .getName()
                .equals(AsfFieldKey.BANNER_IMAGE.getFieldName())
            ) {
  ...
  public static MetadataContainer[] distributeMetadata(final AsfTag tag) {
    ...
    final MetadataContainer[] createContainers =
      ...
      for (int i = 0; !assigned && i < createContainers.length; i++) {
        if (
          ContainerType.areInCorrectOrder(
            createContainers[i].getContainerType(),
