...
abstract class ChunkContainerReader<ChunkType extends ChunkContainer>
   ...
   */
  protected abstract ChunkType createContainer(
    long streamPosition,
  ...
  public ChunkType read(
    ...
    while (currentPosition < result.getChunkEnd()) {
      ...
      final boolean skip =
        this.eachChunkOnce &&
        (!isReaderAvailable(currentGUID) || !alreadyRead.add(currentGUID));
      Chunk chunk;
      /*
      ...
      if (!skip && isReaderAvailable(currentGUID)) {
        ...
        chunk = ChunkHeaderReader.getInstance().read(
          currentGUID,
          cis,
          currentPosition
        );
