...
public class AsfStreamer {
   ...
   * @throws IOException on I/O errors.
   */
  public void createModifiedCopy(
    final InputStream source,
    final OutputStream dest,
    final List<ChunkModifier> modifiers
  ) throws IOException {
        ...
        if (GUID.GUID_FILE.equals(curr)) {
          ...
          for (int j = 0; j < modders.size() && !handled; j++) {
            if (modders.get(j).isApplicable(curr)) {
              // alter current chunk
              final ModificationResult result = modders
                .get(j)
   ...
   * @throws IOException on I/O errors.
   */
  private void modifyFileHeader(
    final InputStream source,
    final OutputStream destination,
    final long fileSizeDiff
  ) throws IOException {
