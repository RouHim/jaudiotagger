...
public class MetadataReader implements ChunkReader {
  ...
  public Chunk read(
    ...
    final MetadataContainer result = new MetadataContainer(
      guid,
      streamPosition,
      chunkLen
    );
    ...
    final boolean isExtDesc =
      result.getContainerType() == ContainerType.EXTENDED_CONTENT;
    ...
    for (int i = 0; i < recordCount; i++) {
      ...
      if (!isExtDesc) {
        ...
        assert result.getContainerType() ==
      ...
      switch (dataType) {
        case MetadataDescriptor.TYPE_STRING:
          descriptor.setStringValue(
            Utils.readFixedSizeUTF16Str(stream, (int) dataLen)
          );
          break;
        ...
        default:
          ...
          descriptor.setStringValue(
            "Invalid datatype: " + new String(Utils.readBinary(stream, dataLen))
          );
      }
