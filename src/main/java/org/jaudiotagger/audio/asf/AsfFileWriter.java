...
public class AsfFileWriter extends AudioFileWriter {
    ...
    protected void deleteTag(
            ...
            final RandomAccessFile tempRaf
    ...
    protected void writeTag(
            ...
            final Tag tag,
            final RandomAccessFile raf,
            final RandomAccessFile rafTemp
    ) throws IOException {
        /*
        ...
        final MetadataContainer[] distribution = TagConverter.distributeMetadata(
                copy
        );
        final boolean[] existHeader = searchExistence(sourceHeader, distribution);
        final boolean[] existExtHeader = searchExistence(
                sourceHeader.getExtendedHeader(),
                distribution
        );
        ...
        new AsfStreamer().createModifiedCopy(
    ...
    private boolean[] searchExistence(
            final ChunkContainer container,
        ...
        for (int i = 0; i < result.length; i++) {
            result[i] = container.hasChunkByGUID(
                    metaContainers[i].getContainerType().getContainerGUID()
            );
        }
        return result;
    }
