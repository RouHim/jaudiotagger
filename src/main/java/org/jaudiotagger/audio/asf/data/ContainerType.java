...
public enum ContainerType {
    ...
    ContainerType(
            final GUID guid,
            final int maxDataLenBits,
            final boolean guidAllowed,
            final boolean stream,
            final boolean language,
            final boolean multiValue
    ) {
        this.containerGUID = guid;
        this.maximumDataLength = BigInteger.valueOf(2)
                .pow(maxDataLenBits)
                .subtract(BigInteger.ONE);
        if (
                this.maximumDataLength.compareTo(BigInteger.valueOf(Long.MAX_VALUE)) <= 0
        ) {
     ...
     */
    public static boolean areInCorrectOrder(
            final ContainerType low,
    ...
    public void assertConstraints(
        ...
        final RuntimeException result = checkConstraints(
                name,
                data,
                type,
                stream,
     ...
     */
    public RuntimeException checkConstraints(
            final String name,
            final byte[] data,
            final int type,
            final int stream,
        ...
        if (
                result == null &&
                        (stream < 0 ||
                                stream > MetadataDescriptor.MAX_STREAM_NUMBER ||
                                (!isStreamNumberEnabled() && stream != 0))
        ) {
        ...
        if (
                result == null && type == MetadataDescriptor.TYPE_GUID && !isGuidEnabled()
        ) {
            result = new IllegalArgumentException(
        ...
        if (
                result == null &&
                        ((language != 0 && !isLanguageEnabled()) ||
                                (language < 0 || language >= MetadataDescriptor.MAX_LANG_INDEX))
        ) {
    ...
    public boolean isWithinValueRange(final long value) {
        return (
                (this.perfMaxDataLen == -1 || this.perfMaxDataLen >= value) && value >= 0
        );
    }
