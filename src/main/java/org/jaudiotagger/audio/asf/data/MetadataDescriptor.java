...
public class MetadataDescriptor
        implements Comparable<MetadataDescriptor>, Cloneable {
     ...
     */
    public static final long DWORD_MAXVALUE = new BigInteger(
            "FFFFFFFF",
            16
    ).longValue();
    /**
    ...
    public void setDWordValue(final long value) {
        if (value < 0 || value > DWORD_MAXVALUE) {
            throw new IllegalArgumentException(
                    "value out of range (0-" + DWORD_MAXVALUE + ")"
            );
        }
            ...
            if (getContainerType().isWithinValueRange(tmp.length)) {
                ...
                if (TagOptionSingleton.getInstance().isTruncateTextWithoutErrors()) {
                    ...
                    final int copyBytes = (int) getContainerType()
                            ...
                            .longValue();
                    this.content = new byte[copyBytes % 2 == 0
                            ? copyBytes
                            : copyBytes - 1];
    ...
    public void setWordValue(final int value) throws IllegalArgumentException {
        if (value < 0 || value > WORD_MAXVALUE) {
            throw new IllegalArgumentException(
                    "value out of range (0-" + WORD_MAXVALUE + ")"
            );
        }
    ...
    public boolean equals(final Object obj) {
        ...
        if (obj instanceof MetadataDescriptor other) {
            if (obj == this) {
                result = true;
            } else {
                result =
     ...
     */
    public int writeInto(final OutputStream out, final ContainerType contType)
            throws IOException {
    ...
    public void setQWordValue(final long value) {
        if (value < 0) {
            throw new IllegalArgumentException(
                    "value out of range (0-" + MetadataDescriptor.QWORD_MAXVALUE + ")"
            );
        }
