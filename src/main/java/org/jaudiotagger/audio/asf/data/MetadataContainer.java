...
public class MetadataContainer extends Chunk implements WriteableChunk {
    ...
    private final Map<DescriptorPointer, List<MetadataDescriptor>> descriptors =
            new Hashtable<DescriptorPointer, List<MetadataDescriptor>>();
    /**
    ...
    private static ContainerType determineType(final GUID guid)
        ...
        if (result == null) {
            throw new IllegalArgumentException(
                    "Unknown metadata container specified by GUID (" + guid + ")"
            );
        }
        return result;
    }
     ...
     */
    public final List<MetadataDescriptor> getDescriptorsByName(
            final String name
    ) {
        assert name != null;
        final List<MetadataDescriptor> result = new ArrayList<MetadataDescriptor>();
        final Collection<List<MetadataDescriptor>> values =
    ...
    public final void removeDescriptorsByName(final String name) {
        assert name != null;
        final Iterator<List<MetadataDescriptor>> iterator =
                this.descriptors.values().iterator();
     ...
     */
    protected final MetadataDescriptor assertDescriptor(
            final String key,
            final int type
    ) {
        MetadataDescriptor desc;
        final List<MetadataDescriptor> descriptorsByName = getDescriptorsByName(
                key
        );
    ...
    public final void addDescriptor(final MetadataDescriptor toAdd)
        ...
        this.containerType.assertConstraints(
                toAdd.getName(),
                toAdd.getRawData(),
                toAdd.getType(),
                toAdd.getStreamNumber(),
                toAdd.getLanguageIndex()
        );
    ...
    public boolean isAddSupported(final MetadataDescriptor descriptor) {
        boolean result =
                getContainerType().checkConstraints(
                        descriptor.getName(),
                        descriptor.getRawData(),
                        descriptor.getType(),
                        descriptor.getStreamNumber(),
                        descriptor.getLanguageIndex()
                ) ==
                        null;
        // Now check if there is already a value contained.
        if (result && !getContainerType().isMultiValued()) {
            synchronized (this.perfPoint) {
                final List<MetadataDescriptor> list = this.descriptors.get(
                        this.perfPoint.setDescriptor(descriptor)
                );
                if (list != null) {
                    result = list.isEmpty();
                }
    ...
    private static final class DescriptorPointer {
         ...
         */
        private DescriptorPointer setDescriptor(
