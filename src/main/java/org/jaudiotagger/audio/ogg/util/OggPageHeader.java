...
public class OggPageHeader {
    ...
    protected static final Logger log = LoggerFactory.getLogger(OggPageHeader.class);
    ...
    private final transient List<PacketStartAndLength> packetList =
            new ArrayList<>();
    ...
    public OggPageHeader(byte[] b) {
        ...
        if (streamStructureRevision == 0x0) {
            this.absoluteGranulePosition = Utils.getLongLE(
                    b,
                    FIELD_ABSOLUTE_GRANULE_POS,
                    13
            );
            this.streamSerialNumber = Utils.getIntLE(
                    b,
                    FIELD_STREAM_SERIAL_NO_POS,
                    17
            );
            this.pageSequenceNumber = Utils.getIntLE(
                    b,
            ...
            for (int i = 0; i < segmentTable.length; i++) {
                ...
                if (segmentLength < MAXIMUM_SEGMENT_SIZE) {
                    packetList.add(
                            new PacketStartAndLength(pageLength - packetLength, packetLength)
                    );
                    packetLength = 0;
                }
            }
        ...
        if (log.isDebugEnabled()) {
            log.debug("Constructed OggPage: " + this);
        }
    }
    ...
    public static OggPageHeader createCommentHeader(
            int pageSize,
            boolean continued,
            int serial,
            int sequence
    ...
    public static OggPageHeader read(ByteBuffer byteBuffer)
        ...
        log.debug("Trying to read OggPage at:" + start);

        byte[] b = new byte[OggPageHeader.CAPTURE_PATTERN.length];
        byteBuffer.get(b);
        if (!(Arrays.equals(b, OggPageHeader.CAPTURE_PATTERN))) {
            throw new CannotReadException(
                    ErrorMessage.OGG_HEADER_CANNOT_BE_FOUND.getMsg(new String(b))
            );
        }
     ...
     * @throws CannotReadException
     */
    public static OggPageHeader read(RandomAccessFile raf)
        ...
        if (!(Arrays.equals(b, OggPageHeader.CAPTURE_PATTERN))) {
            ...
            if (AbstractID3v2Tag.isId3Tag(raf)) {
                log.warn(
                        ErrorMessage.OGG_CONTAINS_ID3TAG.getMsg(raf.getFilePointer() - start)
                );
                raf.read(b);
                if ((Arrays.equals(b, OggPageHeader.CAPTURE_PATTERN))) {
    ...
    public long getAbsoluteGranulePosition() {
        log.debug("Number Of Samples: " + absoluteGranulePosition);
    ...
    public void setChecksum(int checksum) {
        ByteBuffer.wrap(rawHeaderData)
                .order(ByteOrder.LITTLE_ENDIAN)
                .putInt(FIELD_PAGE_CHECKSUM_POS, checksum);
    ...
    public int getPageLength() {
        log.debug("This page length: " + pageLength);
    ...
    public String toString() {
        String out =
                "Ogg Page Header { isValid: " +
                        isValid +
                        ", type: " +
                        headerTypeFlag +
                        ", oggPageHeaderLength: " +
                        rawHeaderData.length +
                        ", length: " +
                        pageLength +
                        ", seqNo: " +
                        getPageSequence() +
    ...
    public void setPageSequence(int seqNo) {
        ByteBuffer.wrap(rawHeaderData)
                .order(ByteOrder.LITTLE_ENDIAN)
                .putInt(FIELD_PAGE_SEQUENCE_NO_POS, seqNo);
