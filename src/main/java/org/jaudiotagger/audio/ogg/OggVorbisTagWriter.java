...
public class OggVorbisTagWriter {
    ...
    public void write(Tag tag, RandomAccessFile raf, RandomAccessFile rafTemp)
        ...
        log.debug("Read 1st Page:identificationHeader:");
        ...
        log.debug("Written identificationHeader:");
        ...
        log.debug("Old 2nd Page size: " + secondPageHeader.getPageLength());
        log.debug("Old last page status: " + secondPageHeader.isLastPage());
        log.debug(
        ...
        log.debug("Old comment: " + vorbisHeaderSizes.getCommentHeaderSize());
        log.debug("New comment: " + newCommentLength);
        log.debug("New Page Data Size: " + newSecondPageDataLength);
        ...
        if (
            ...
            if (
                ...
                log.debug("Header and Setup remain on single page:");
            ...
            //Original 2nd page spanned multiple pages so more work to do
                ...
                log.debug("Header and Setup now on single page:");
        ...
        //Bit more complicated, have to create more than one new page and renumber subsequent audio
            ...
            log.debug("Header and Setup with shift audio:");
    ...
    private ByteBuffer startCreateBasicSecondPage(
        ...
        log.debug("New second page header length:" + newSecondPageHeaderLength);
        log.debug("No of segments:" + segmentTable.length);
    ...
    private void replacePagesAndRenumberPageSeqs(
        ...
        int lastPageCommentPacketSize =
                newCommentLength % OggPageHeader.MAXIMUM_PAGE_DATA_SIZE;
        log.debug("Last comment packet size:" + lastPageCommentPacketSize);
        ...
        if (
            ...
            //Write the last part of comment only (its possible it might be the only comment)
            {
                ...
                log.debug("Writing Last Comment Page " + pageSequence + " to file");
                pageSequence++;
                calculateChecksumOverPage(pageBuffer);
                rafTemp.getChannel().write(pageBuffer);
            }
    ...
    private byte[] createSegmentTable(
        ...
        log.debug("Created " + newStart.length + " segments for header");
        log.debug("Created " + restShouldBe.length + " segments for setup");
    ...
    private boolean isCommentAndSetupHeaderFitsOnASinglePage(
        ...
        log.debug("Require:" + totalDataSize + " segments for comment");
