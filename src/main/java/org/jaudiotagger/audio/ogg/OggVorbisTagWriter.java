...
public class OggVorbisTagWriter {
  ...
  private final OggVorbisCommentTagCreator tc =
    new OggVorbisCommentTagCreator();
  ...
  public void write(Tag tag, RandomAccessFile raf, RandomAccessFile rafTemp)
    ...
    logger.debug("Written identificationHeader:");
    ...
    logger.debug(
      "Read 2nd Page:comment and setup and possibly audio:Header finishes at file position:" +
    ...
    OggVorbisTagReader.OggVorbisHeaderSizes vorbisHeaderSizes =
      reader.readOggVorbisHeaderSizes(raf);
    ...
    int newSecondPageDataLength =
      ...
      newCommentLength +
      vorbisHeaderSizes.getExtraPacketDataSize();
    logger.debug(
      "Old 2nd Page no of packets: " + secondPageHeader.getPacketList().size()
    );
    logger.debug("Old 2nd Page size: " + secondPageHeader.getPageLength());
    logger.debug("Old last page status: " + secondPageHeader.isLastPage());
    logger.debug(
      "Setup Header Size: " + vorbisHeaderSizes.getSetupHeaderSize()
    );
    logger.debug(
      "Extra Packets: " + vorbisHeaderSizes.getExtraPacketList().size()
    );
    logger.debug(
      "Extra Packet Data Size: " + vorbisHeaderSizes.getExtraPacketDataSize()
    );
    logger.debug("Old comment: " + vorbisHeaderSizes.getCommentHeaderSize());
    logger.debug("New comment: " + newCommentLength);
    logger.debug("New Page Data Size: " + newSecondPageDataLength);
    ...
    if (
      ...
      if (
        (secondPageHeader.getPageLength() <
          OggPageHeader.MAXIMUM_PAGE_DATA_SIZE) &&
        (((secondPageHeader.getPacketList().size() == 2) &&
            secondPageHeader.isLastPage()) ||
          (secondPageHeader.getPacketList().size() > 2))
      ) {
        logger.debug("Header and Setup remain on single page:");
      ...
      //Original 2nd page spanned multiple pages so more work to do
        ...
        logger.debug("Header and Setup now on single page:");
    ...
    //Bit more complicated, have to create more than one new page and renumber subsequent audio
      ...
      logger.debug("Header and Setup with shift audio:");
  ...
  private ByteBuffer startCreateBasicSecondPage(
    ...
    logger.debug("WriteOgg Type 1");
    byte[] segmentTable = createSegmentTable(
      newCommentLength,
      vorbisHeaderSizes.getSetupHeaderSize(),
      vorbisHeaderSizes.getExtraPacketList()
    );
    int newSecondPageHeaderLength =
      OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
    logger.debug("New second page header length:" + newSecondPageHeaderLength);
    logger.debug("No of segments:" + segmentTable.length);

    ByteBuffer secondPageBuffer = ByteBuffer.allocate(
    ...
    secondPageBuffer.put(
      secondPageHeader.getRawHeaderData(),
      0,
  ...
  private void replaceSecondPageOnly(
    ...
    logger.debug("WriteOgg Type 1");
    ByteBuffer secondPageBuffer = startCreateBasicSecondPage(
      vorbisHeaderSizes,
      newCommentLength,
      newSecondPageLength,
      secondPageHeader,
   ...
   */
  private void replaceSecondPageAndRenumberPageSeqs(
    OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes,
    int newCommentLength,
    int newSecondPageLength,
    OggPageHeader secondPageHeader,
    ByteBuffer newComment,
    ...
    byte[] setupHeaderData =
      reader.convertToVorbisSetupHeaderPacketAndAdditionalPackets(
        originalHeaderSizes.getSetupHeaderStartPosition(),
        raf
      );
    logger.debug(
      setupHeaderData.length +
        ":" +
        secondPageBuffer.position() +
        ":" +
        secondPageBuffer.capacity()
    );
   ...
   */
  private void replacePagesAndRenumberPageSeqs(
    OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes,
    int newCommentLength,
    OggPageHeader secondPageHeader,
    ByteBuffer newComment,
    ...
    int noOfCompletePagesNeededForComment =
      newCommentLength / OggPageHeader.MAXIMUM_PAGE_DATA_SIZE;
    logger.debug(
      "Comment requires:" +
        noOfCompletePagesNeededForComment +
        " complete pages"
    );
    ...
    if (noOfCompletePagesNeededForComment > 0) {
      for (int i = 0; i < noOfCompletePagesNeededForComment; i++) {
        ...
        byte[] segmentTable = this.createSegments(
          OggPageHeader.MAXIMUM_PAGE_DATA_SIZE,
          false
        );
        int pageHeaderLength =
          OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
        ByteBuffer pageBuffer = ByteBuffer.allocate(
          pageHeaderLength + OggPageHeader.MAXIMUM_PAGE_DATA_SIZE
        );
        ...
        pageBuffer.putInt(
          OggPageHeader.FIELD_PAGE_SEQUENCE_NO_POS,
          pageSequence
        );
        pageSequence++;
    ...
    int lastPageCommentPacketSize =
      newCommentLength % OggPageHeader.MAXIMUM_PAGE_DATA_SIZE;
    logger.debug("Last comment packet size:" + lastPageCommentPacketSize);
    ...
    if (
      ...
      //Write the last part of comment only (its possible it might be the only comment)
      {
        ...
        int pageHeaderLength =
          OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
        ByteBuffer pageBuffer = ByteBuffer.allocate(
          lastPageCommentPacketSize + pageHeaderLength
        );
        pageBuffer.order(ByteOrder.LITTLE_ENDIAN);
        pageBuffer.put(
          secondPageHeader.getRawHeaderData(),
        ...
        logger.debug("Writing Last Comment Page " + pageSequence + " to file");
        pageSequence++;
        calculateChecksumOverPage(pageBuffer);
        rafTemp.getChannel().write(pageBuffer);
      }
      ...
      {
        byte[] segmentTable = this.createSegmentTable(
          originalHeaderSizes.getSetupHeaderSize(),
          originalHeaderSizes.getExtraPacketList()
        );
        int pageHeaderLength =
        ...
        ByteBuffer pageBuffer = ByteBuffer.allocate(
          setupHeaderData.length + pageHeaderLength
        );
        pageBuffer.order(ByteOrder.LITTLE_ENDIAN);
        pageBuffer.put(
          secondPageHeader.getRawHeaderData(),
   ...
   * @throws CannotWriteException
   */
  public void writeRemainingPages(
    int pageSequence,
    RandomAccessFile raf,
    RandomAccessFile rafTemp
  ) throws IOException, CannotReadException, CannotWriteException {
    ...
    ByteBuffer bb = ByteBuffer.allocate(
      (int) (raf.length() - raf.getFilePointer())
    );
    ByteBuffer bbTemp = ByteBuffer.allocate(
      (int) (raf.length() - raf.getFilePointer())
    );
  ...
  private byte[] createSegmentTable(
    ...
    logger.debug("Created " + newStart.length + " segments for header");
    logger.debug("Created " + restShouldBe.length + " segments for setup");
    ...
    try {
      ...
      if (extraPackets.size() > 0) {
        ...
        logger.debug(
          "Creating segments for " + extraPackets.size() + " packets"
        );
      ...
      throw new RuntimeException(
        "Unable to create segment table:" + ioe.getMessage()
      );
    }
  ...
  private byte[] createSegments(int length, boolean quitStream) {
    ...
    ((length % OggPageHeader.MAXIMUM_SEGMENT_SIZE == 0 && !quitStream)
        ? 0
        : 1)];
    int i = 0;
    ...
    result[result.length - 1] = (byte) (length -
      (i * OggPageHeader.MAXIMUM_SEGMENT_SIZE));
    return result;
  }
   ...
   */
  private boolean isCommentAndSetupHeaderFitsOnASinglePage(
    int commentLength,
    int setupHeaderLength,
    ...
    logger.debug("Require:" + totalDataSize + " segments for comment");

    if (setupHeaderLength == 0) {
      totalDataSize++;
    } else {
      totalDataSize +=
        (setupHeaderLength / OggPageHeader.MAXIMUM_SEGMENT_SIZE) + 1;
    ...
    for (OggPageHeader.PacketStartAndLength extraPacket : extraPacketList) {
      if (extraPacket.getLength() == 0) {
        ...
        totalDataSize +=
          (extraPacket.getLength() / OggPageHeader.MAXIMUM_SEGMENT_SIZE) + 1;
    ...
    logger.debug(
      "Total No Of Segment If New Comment And Header Put On One Page:" +
        totalDataSize
    );
