...
public class OggVorbisTagReader {
    ...
    public Tag read(RandomAccessFile raf)
        ...
        log.debug("CompletedReadCommentTag");
        return tag;
    }
     ...
     * @throws IOException
     */
    public byte[] readRawPacketData(RandomAccessFile raf)
            throws CannotReadException, IOException {
        log.debug("Read 1st page");
        //1st page = codec infos
        OggPageHeader pageHeader = OggPageHeader.read(raf);
        //Skip over data to end of page header 1
        raf.seek(raf.getFilePointer() + pageHeader.getPageLength());

        log.debug("Read 2nd page");
        ...
        byte[] b = new byte[VorbisHeader.FIELD_PACKET_TYPE_LENGTH +
    ...
    public boolean isVorbisCommentHeader(byte[] headerData) {
        ...
        return !(
                headerData[VorbisHeader.FIELD_PACKET_TYPE_POS] !=
                        VorbisPacketType.COMMENT_HEADER.getType() ||
    ...
    protected byte[] convertToVorbisCommentPacket(
        ...
        if (startVorbisCommentPage.getPacketList().size() > 1) {
            log.debug(
                    "Comments finish on 2nd Page because there is another packet on this page"
            );
        ...
        while (true) {
            ...
            if (currentHeader.getPacketList().size() > 1) {
                log.debug(
                        "Comments finish on Page because there is another packet on this page"
                );
                return baos.toByteArray();
            }
            ...
            if (!currentHeader.isLastPacketIncomplete()) {
                log.debug("Comments finish on Page because this packet is complete");
                return baos.toByteArray();
            }
        }
    }
    ...
    public byte[] convertToVorbisSetupHeaderPacketAndAdditionalPackets(
            long fileOffsetOfStartingOggPage,
            RandomAccessFile raf
    ) throws IOException, CannotReadException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        ...
        if (!isVorbisSetupHeader(b)) {
            throw new CannotReadException(
                    "Unable to find setup header(2), unable to write ogg file"
            );
        }
        ...
        //Return Data
        if (
                !setupPageHeader.isLastPacketIncomplete() ||
                        setupPageHeader.getPacketList().size() > 2
        ) {
            log.debug("Setupheader finishes on this page");
        ...
        //so carry on reading pages until we get to the end of comment
        while (true) {
            log.debug("Reading another page");
            OggPageHeader currentHeader = OggPageHeader.read(raf);
            b = new byte[currentHeader.getPacketList().get(0).getLength()];
            raf.read(b);
            baos.write(b);
            ...
            //on this page so thats all we need and we can return
            if (currentHeader.getPacketList().size() > 1) {
                log.debug("Setup header finishes on this page");
                return baos.toByteArray();
            }
            ...
            if (!currentHeader.isLastPacketIncomplete()) {
                log.debug(
                        "Setup header finish on Page because this packet is complete"
                );
                return baos.toByteArray();
            }
        }
    ...
    public boolean isVorbisSetupHeader(byte[] headerData) {
        ...
        return !(
                headerData[VorbisHeader.FIELD_PACKET_TYPE_POS] !=
                        VorbisPacketType.SETUP_HEADER.getType() ||
                        !vorbis.equals(VorbisHeader.CAPTURE_PATTERN)
        );
    }
    ...
    public OggVorbisHeaderSizes readOggVorbisHeaderSizes(RandomAccessFile raf)
            throws CannotReadException, IOException {
        log.debug("Started to read comment and setup header sizes:");
        ...
        while (true) {
            ...
            if (packetList.size() > 1 || !pageHeader.isLastPacketIncomplete()) {
                //done comment size
                log.debug(
                        "Found end of comment:size:" +
                                commentHeaderSize +
                                "finishes at file position:" +
                                raf.getFilePointer()
                );
                break;
            }
        ...
        if (pageHeader.getPacketList().size() == 1) {
            ...
            b = new byte[VorbisHeader.FIELD_PACKET_TYPE_LENGTH +
                    VorbisHeader.FIELD_CAPTURE_PATTERN_LENGTH];
            raf.read(b);
            if (!isVorbisSetupHeader(b)) {
                throw new CannotReadException(
                        ErrorMessage.OGG_VORBIS_NO_VORBIS_HEADER_FOUND.getMsg()
                );
            }
            raf.seek(
                    raf.getFilePointer() -
            ...
            log.debug(
                    "Found start of vorbis setup header at file position:" +
                            raf.getFilePointer()
            );
            ...
            log.debug("Adding:" + packet.getLength() + " to setup header size");
            ...
            //The setup header continues onto the next page
                ...
                while (true) {
                    ...
                    log.debug(
                            "Adding:" + packetList.get(0).getLength() + " to setup header size"
                    );
                    raf.skipBytes(packetList.get(0).getLength());
                    if (packetList.size() > 1 || !pageHeader.isLastPacketIncomplete()) {
                        //done setup size
